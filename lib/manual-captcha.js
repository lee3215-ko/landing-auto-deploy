...
export async function runManualCaptchaAndCollect({
  ...
  const recordAndFinishMetaMiss = async (popupMsg) => {
    const result = {
      ...metaMissingResult(url, popupMsg),
      offerDelete: true,
      stopManualCaptcha: true,
    };
    relay(sendLog, '⏹ 메타미검출 — 수동캡챠 즉시 중단 (삭제 여부 확인)');
    ...
    if (!hasHtmlTitle && (await isOwnershipDone(page, dlg))) {
      ...
      while (Date.now() - start < waitTimeoutMs) {
        ...
        if (popupMsg) {
          // 메타미검출 즉시 중단 — 자동 재배포/대기 없이 삭제 확인으로 넘김
          relay(sendLog, `📋 메타미검출 팝업: "${popupMsg.slice(0, 120)}"`);
