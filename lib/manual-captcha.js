...
export async function runManualCaptchaAndCollect({
  ...
  relay(sendLog, '기존 네이버 Chrome에 새 탭을 열어 진행합니다. (생성 중인 탭은 그대로 둡니다)');

  // 메인 탭(사이트 생성용)을 건드리지 않고 세션만 확인
  const session = await ensureNaverSession({
    naverAccount: naverAccount?.id && naverAccount?.pw ? naverAccount : null,
    openaiApiKey,
    yesCaptchaClientKey,
    headless: false,
  ...
  const mainPage = peekNaverSessionPage() || session?.page || null;
  if (!browser) throw new Error('네이버 세션 페이지가 없습니다. 우측 상단 「네이버 로그인」을 먼저 해주세요.');

  // 새 탭에서만 수동캡챠·수집 진행 — 메인 세션 탭 핸들은 복구
  let page;
  try {
  ...
  try {
    ...
    const collectOut = await submitNaverBulkCollection({
      ...
      naverAccount: session?.naverAccount || naverAccount,
      openaiApiKey,
      yesCaptchaClientKey,
      outputRoot,
      sendLog,
