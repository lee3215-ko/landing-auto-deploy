...
const SITE_CREATE_GAP_MS = 18000;
...
/** Netlify API 오류를 사용자용 한글 메시지로 */
export function formatNetlifyApiError(status, errText, action = 'API') {
  const raw = String(errText || '').trim();
  const isRate = status === 429
    || /rate limit|API Deploy rate limit|surpassed for application/i.test(raw);
  if (isRate) {
    return (
      'Netlify API 배포 호출 한도를 초과했습니다 (429).\n'
      + '같은 계정/토큰으로 사이트 생성·배포를 너무 빠르게 연속 호출하면 잠시 차단됩니다.\n'
      + '대안: ① 5~15분 대기 후 재시도 ② 한 번에 만드는 개수를 줄이기 ③ 사이트 사이 간격을 늘리기 '
      + '④ 다른 Netlify 팀/토큰 사용(같은 팀이면 한도 공유).\n'
      + '크레딧 부족이 아니라 API 속도 제한입니다.'
    );
  }
  if (status === 401 || status === 403) {
    return `Netlify 인증 실패 (${status}). 토큰을 다시 발급·등록하세요.`;
  }
  const short = raw.replace(/\s+/g, ' ').slice(0, 180);
  return `Netlify ${action} 실패 (${status})${short ? `: ${short}` : ''}`;
}

function throwNetlifyHttpError(status, errText, action) {
  throw new Error(formatNetlifyApiError(status, errText, action));
}
...
async function apiGet(endpoint, token, attempt = 1) {
  ...
  if (resp.status === 429 && attempt < MAX_RATE_RETRIES) {
    ...
    warn(`Netlify 조회 한도(429) — ${Math.round(wait / 1000)}초 후 재시도 (${attempt}/${MAX_RATE_RETRIES - 1})…`);
    await sleep(wait);
  ...
  if (!resp.ok) {
    ...
    throwNetlifyHttpError(resp.status, errText || resp.statusText, '조회');
  }
...
async function apiPost(endpoint, token, bodyObj, attempt = 1) {
  ...
  if (resp.status === 429 && attempt < MAX_RATE_RETRIES) {
    const wait = rateLimitWaitMs(attempt, resp);
    warn(`Netlify API 한도(429) — ${Math.round(wait / 1000)}초 후 재시도 (${attempt}/${MAX_RATE_RETRIES - 1})…`);
    await sleep(wait);
  ...
  if (!resp.ok) {
    ...
    throwNetlifyHttpError(resp.status, errText, endpoint.includes('/sites') ? '사이트 생성' : '요청');
  }
...
async function apiPatch(endpoint, token, bodyObj, attempt = 1) {
  ...
  if (!resp.ok) {
    ...
    throwNetlifyHttpError(resp.status, errText, '사이트 설정 변경');
  }
...
async function uploadDeployZip(siteId, token, zipBuf, attempt = 1) {
  ...
  if (resp.status === 429 && attempt < MAX_RATE_RETRIES) {
    const wait = rateLimitWaitMs(attempt, resp);
    warn(`Netlify 배포 한도(429) — ${Math.round(wait / 1000)}초 후 재시도 (${attempt}/${MAX_RATE_RETRIES - 1})…`);
    await sleep(wait);
  ...
  if (!resp.ok) {
    ...
    throwNetlifyHttpError(resp.status, errText, '배포 업로드');
  }
...
export async function deploySite({ netlifyToken, siteName, siteId, dir, zipPath, serviceName }) {
  ...
  if (zipPath) {
    ...
    // 실제 Netlify 공개 URL 기준으로 sitemap/robots/canonical 등 재작성 후 업로드
    const publicTarget = sitePublicUrl(site).replace(/\/$/, '');
    if (publicTarget) {
      try {
        const { rewriteSitePublicUrls } = await import('./source-utils.js');
        rewriteSitePublicUrls(deployRoot, publicTarget, (m) => log(`[${serviceName}] ${m}`));
      } catch (e) {
        warn(`[${serviceName}] 사이트 URL 재작성 경고: ${e.message}`);
      }
    }
