...
export async function registerNaverMetaForKkangSite({
  ...
  const results = await registerNaverSites({
    ...
    redeployCallback: async (_site, metaTag) => {
      ...
      // ZIP/폴더에 남은 예전 도메인 → 등록 URL로 재작성 (deploySite에서도 실제 URL로 재확인)
      try {
        const { rewriteSitePublicUrls } = await import('./source-utils.js');
        rewriteSitePublicUrls(dir, rootUrl.replace(/\/$/, ''), log);
      } catch (e) {
        log(`⚠ 사이트 URL 재작성 경고: ${e.message}`);
      }
      if (firstDeploy) {
