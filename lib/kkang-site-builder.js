...
export async function registerNaverMetaForKkangSite({
  ...
  try {
    const { countAdvisorRegisteredSites, getNaverSessionPage } = await import('./naver-session.js');
    const sessionPage = await getNaverSessionPage();
    const n = await countAdvisorRegisteredSites(sessionPage);
    if (n != null) log(`서치어드바이저 등록 사이트: ${n}개`);
  } catch { /* ignore */ }

  // 하위 페이지 웹페이지 수집 (URL 수집 탭과 동일 로직)
