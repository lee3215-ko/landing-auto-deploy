...
export async function loginNaverWithCaptcha(page, {
  ...
  // IP보안 ON이면 VPN 전환 후 메일 세션이 끊김 → 로그인 전 OFF
  try {
    await page.evaluate(() => {
      const candidates = [
        ...
        ...Array.from(document.querySelectorAll('.ip_check .switch, .ip_check [role="checkbox"], .switch_on')),
      ].filter(Boolean);
  ...
  while (Date.now() - start < 120000) {
    try {
      const { detectNaverLoginRestriction, isMassCreatedIdRestriction, NaverMassCreatedIdError } = await import('./naver-login-wait.js');
      const info = await detectNaverLoginRestriction(page);
      if (isMassCreatedIdRestriction(info)) {
        throw new NaverMassCreatedIdError({ ...info, accountId: info.accountId || naverId });
      }
    } catch (e) {
      if (e?.code === 'NAVER_MASS_CREATED_ID' || e?.name === 'NaverMassCreatedIdError') throw e;
    }
