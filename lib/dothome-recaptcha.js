...
async function solveImageChallengeLoop(page, apiKey, folder, sendLog, _maxChallengeRounds = 1) {
  ...
  // OpenAI Vision은 reCAPTCHA 타일을 정책상 거부(numbers=none) → 시간만 쓰고 만료됨
  // 자동 통과는 YesCaptcha 토큰 주입(위에서 이미 시도). 여기선 수동 선택만.
  log('타일 챌린지 — OpenAI Vision 생략(거절률 높음). YesCaptcha 없으면 창에서 타일만 골라 주세요.');
