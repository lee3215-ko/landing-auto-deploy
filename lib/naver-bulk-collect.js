...
async function collectOneSite({
  ...
  if (doWebpage) {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      relay(sendLog, `[${i + 1}/${targets.length}] 웹페이지 수집 요청: ${target}`);
      try {
        page = await ensurePage();
        const res = await submitAdvisorTextRequest(page, crawlPage, target, folder, {
...
export async function submitNaverBulkCollection({
  ...
  relay(sendLog, `모드: ${headless ? '헤드리스(창 숨김)' : '창 표시'}`);
  ...
  const ensurePage = async () => {
    ...
    try {
      ...
      relay(sendLog, '⚠ 페이지/프레임 손상 — 새 탭으로 복구');
  ...
  try {
    ...
    if (hasCreds) {
      relay(sendLog, `네이버 자동 로그인 (URL 수집 전용 계정: ${naverAccount.id})...`);
      await loginNaverForAdvisor(page, naverAccount, { openaiApiKey, outputFolder: folder, manualOnly: false });
    } else {
      relay(sendLog, '네이버 로그인 창 열기 (직접 로그인)...');
      await loginNaverForAdvisor(page, null, { openaiApiKey, outputFolder: folder, manualOnly: true });
    }
