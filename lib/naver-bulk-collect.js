...
async function collectOneSite({
  ...
  // 웹수집 대상에서 sitemap.xml 제외 · 홈 URL 은 항상 맨 앞
  const rawTargets = dedupeUrls(pageUrls.length ? pageUrls : [siteUrl])
    .filter((u) => !/\/sitemap\.xml$/i.test(String(u || '').replace(/\/$/, '')));
  const homeKey = urlKey(siteUrl);
  const targets = rawTargets.some((u) => urlKey(u) === homeKey)
    ? rawTargets
    : [normalizeAdvisorSiteUrl(siteUrl), ...rawTargets].filter(Boolean);
  const stats = {
    ownership: null,
    fast: false,
    robots: false,
  ...
  throwIfCrawlStopped();
  if (skipOwnership) {
    relay(sendLog, `소유확인 생략 (수동캡챠 직후) · 웹수집 ${targets.length}개 URL`);
    stats.ownership = { ok: true, owned: true, neededVerify: false, message: '수동캡챠 완료' };
  } else {
  ...
  throwIfCrawlStopped();
  // 요약 콘솔 강제 이동 제거 — 보호조치 유발. 바로 설정/robots/수집 단계로 진행
  page = await ensurePage();
  relay(sendLog, `사이트 수집 시작 (${siteUrl}) · 하위 URL ${targets.length}개 · 현재: ${safePageUrl(page)}`);
    ...
    try {
      ...
      if (sm.ok) {
        ...
        if (/미등록|권한/.test(sm.reason || '')) {
          // 사이트맵만 실패해도 홈·하위 URL 웹수집은 계속 (동시 수동캡챠 레이스로 폼 일시 누락되는 경우 대비)
          relay(sendLog, '사이트맵 폼 없음/권한 이슈 — 웹페이지 수집은 계속 진행합니다.');
        }
      }
    } catch (e) {
      if (e?.name === 'CrawlStopped' || e?.cancelled) throw e;
  ...
  if (doWebpage) {
    ...
    let crawlFormReady = false;
    for (let i = 0; i < targets.length; i++) {
      await ensureNoNaverProtection(page, {
        log: (msg) => { try { relay(sendLog, msg); } catch { /* ignore */ } },
        source: 'naver-bulk-collect',
      });
      throwIfCrawlStopped();
      const target = targets[i];
      // 경로(/limit-guide.html)만 넣으면 빨간 오류 — 배포 전체 URL 필수
      const crawlUrl = toAdvisorCrawlPageUrl(siteUrl, target);
      relay(sendLog, `[${i + 1}/${targets.length}] 웹페이지 수집: ${crawlUrl}${crawlFormReady ? ' (폼 재사용)' : ''}`);
      try {
        page = await ensurePage();
        // OAuth에 있을 때만 이동 — 정상 수집 폼에서는 no-op
        ...
        const res = await submitAdvisorTextRequest(page, crawlPage, crawlUrl, folder, {
          ...
          // 첫 URL·실패 직후만 강제 진입, 이후는 같은 폼에서 입력만 반복
          forceNavigate: !crawlFormReady,
        });
        if (res.ok) {
          stats.pagesOk += 1;
          consecutiveFails = 0;
      ...
      // 폼 재사용 시 대기만 짧게 — 이동이 없으므로 보호조치 부담↓
      await sleep(crawlFormReady ? 700 : 1200);
    }
...
export async function submitNaverBulkCollection({
  ...
  const folder = path.join(
    ...
    `naver-collect-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
  ...
  try {
    ...
    throwIfCrawlStopped();
    relay(sendLog, '서치어드바이저 보드 확인...');
    await ensureAdvisorPage(page, BOARD, { settleMs: 1200, label: '보드' });
    if (await escapeAdvisorOauthCallback(page, BOARD)) {
      relay(sendLog, 'OAuth 콜백 감지 — 보드로 재이동');
      await ensureAdvisorPage(page, BOARD, { settleMs: 1200, force: true, label: '보드' });
    }
