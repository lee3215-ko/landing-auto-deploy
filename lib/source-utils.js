...
export function rewriteSitePublicUrls(siteDir, siteRootUrl, sendLog = null) {
        ...
        (_, a, href, c) => {
          if (/^https?:\/\//i.test(href)) {
            try {
              ...
              if (/\.dothome\.co\.kr$/i.test(u.host)
                || /\.pages\.dev$/i.test(u.host)
                || /\.netlify\.app$/i.test(u.host)
                || u.pathname.includes('__SITE_URL__')) {
                return `${a}${toAbs(u.pathname)}${c}`;
              }
            } catch { /* keep */ }
            return `${a}${href}${c}`;
  ...
  if (fs.existsSync(smPath)) {
    try {
      ...
      xml = xml.replace(/https?:\/\/[a-z0-9._-]+\.netlify\.app/gi, absRoot);
      xml = xml.replace(/https?:\/\/[a-z0-9._-]+\.netlify\.com/gi, absRoot);
      xml = xml.replace(/https?:\/\/[a-z0-9._-]+\.pages\.dev/gi, absRoot);
