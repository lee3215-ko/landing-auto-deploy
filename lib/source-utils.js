...
/** CDN·외부 리소스 호스트 — 사이트 도메인 치환 대상에서 제외 */
const EXTERNAL_ASSET_HOST_RE = /(?:^|\.)(?:googleapis\.com|gstatic\.com|google\.com|googletagmanager\.com|google-analytics\.com|schema\.org|sitemaps\.org|w3\.org|facebook\.com|fbcdn\.net|twitter\.com|twimg\.com|instagram\.com|cdn\.jsdelivr\.net|unpkg\.com|cdnjs\.cloudflare\.com|bootstrapcdn\.com|fontawesome\.com|jquery\.com|cloudfront\.net|akamaihd\.net|youtube\.com|ytimg\.com|vimeo\.com|github\.io|raw\.githubusercontent\.com|naver\.com|pstatic\.net)$/i;

function escapeRegExp(s) {
  return String(s || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

function isExternalAssetHost(hostname) {
  const h = String(hostname || '').toLowerCase();
  if (!h) return true;
  return EXTERNAL_ASSET_HOST_RE.test(h);
}

/**
 * ZIP/폴더 배포 전: HTML·sitemap·robots 공개 URL을 이번 배포 도메인으로 맞춤.
 * 예전 도메인·다른 호스팅 주소·`__SITE_URL__` 등 플레이스홀더를 치환한다.
...
export function rewriteSitePublicUrls(siteDir, siteRootUrl, sendLog = null) {
  ...
  let host = '';
  try { host = new URL(root).host.toLowerCase(); } catch { host = ''; }
  const absRoot = root;
  const hostNoWww = host.replace(/^www\./i, '');

  /** sitemap/robots/html에 박힌 「이전 사이트」호스트 수집 */
  const foreignHosts = new Set();
  const absorbHostsFromText = (text) => {
    const re = /https?:\/\/([a-z0-9._-]+)/gi;
    let m;
    while ((m = re.exec(String(text || '')))) {
      const h = String(m[1] || '').toLowerCase();
      if (!h || h === host || h.replace(/^www\./, '') === hostNoWww) continue;
      if (isExternalAssetHost(h)) continue;
      foreignHosts.add(h);
    }
  };
  try {
    const smProbe = path.join(dir, 'sitemap.xml');
    if (fs.existsSync(smProbe)) {
      const smText = fs.readFileSync(smProbe, 'utf8');
      // xmlns 등 네임스페이스 URL은 건드리지 않고 <loc>만 수집
      const locs = smText.match(/<loc>\s*([^<\s]+)\s*<\/loc>/gi) || [];
      absorbHostsFromText(locs.join('\n'));
    }
    const rbProbe = path.join(dir, 'robots.txt');
    if (fs.existsSync(rbProbe)) absorbHostsFromText(fs.readFileSync(rbProbe, 'utf8'));
    // HTML 안 canonical/og·절대링크에만 남은 예전 도메인도 수집
    const scanHtmlHosts = (current) => {
      let entries;
      ...
      for (const ent of entries) {
        ...
        if (ent.isDirectory()) {
          if (['node_modules', '.git', '__pycache__', 'img', 'images', 'assets', 'css', 'js'].includes(ent.name)) continue;
          scanHtmlHosts(full);
        } else if (/\.html?$/i.test(ent.name)) {
          try {
            const raw = fs.readFileSync(full, 'utf8');
            const metas = raw.match(
              /rel=["']canonical["'][^>]*>|href=["'][^"']+["']\s+rel=["']canonical["']|property=["']og:(?:url|image)["'][^>]*>/gi,
            );
            if (metas) absorbHostsFromText(metas.join('\n'));
          } catch { /* ignore */ }
        }
      }
    };
  ...
  const replaceKnownHosts = (text) => {
    let out = String(text || '');
    out = out.replace(/https?:\/\/[a-z0-9._-]+\.dothome\.co\.kr/gi, absRoot);
    out = out.replace(/https?:\/\/[a-z0-9._-]+\.netlify\.app/gi, absRoot);
    out = out.replace(/https?:\/\/[a-z0-9._-]+\.netlify\.com/gi, absRoot);
    out = out.replace(/https?:\/\/[a-z0-9._-]+\.pages\.dev/gi, absRoot);
    for (const oldHost of foreignHosts) {
      out = out.replace(new RegExp(`https?:\\/\\/${escapeRegExp(oldHost)}`, 'gi'), absRoot);
    }
    return out;
  };
  ...
  const shouldRewriteSiteHost = (hostname) => {
    const h = String(hostname || '').toLowerCase();
    if (!h) return false;
    if (h === host || h.replace(/^www\./, '') === hostNoWww) return false;
    if (isExternalAssetHost(h)) return false;
    if (/\.(?:dothome\.co\.kr|netlify\.app|netlify\.com|pages\.dev)$/i.test(h)) return true;
    if (foreignHosts.has(h)) return true;
    return false;
  };
        ...
        (_, a, href, c) => {
          if (/^https?:\/\//i.test(href)) {
            try {
              ...
              if (shouldRewriteSiteHost(u.host) || u.pathname.includes('__SITE_URL__')) {
                return `${a}${toAbs(u.pathname)}${c}`;
              }
            } catch { /* keep */ }
            return `${a}${href}${c}`;
  ...
  log(`사이트 공개 URL 반영: ${absRoot}/ (html ${htmlFiles} · sitemap ${sitemap ? 'OK' : '-'} · robots ${robots ? 'OK' : '-'}${foreignHosts.size ? ` · 이전호스트 ${foreignHosts.size}` : ''})`);
...
/**
 * FTP에 올린(URL 치환·메타 반영된) 로컬 폴더를 `성공/{원본zip명}` 으로 다시 압축 저장.
 * 원본 ZIP은 삭제(또는 실패 시 이동). 성공 폴더 ZIP = 실제 업로드본과 동일하게 맞춤.
 * @returns {Promise<{ ok: boolean, from?: string, path?: string, skipped?: boolean, error?: string, rewritten?: boolean }>}
 */
export async function saveRewrittenSiteAsSuccessZip(siteDir, originalZipPath) {
  const from = String(originalZipPath || '').trim();
  const dir = String(siteDir || '').trim();
  if (!from) return { ok: false, error: '원본 ZIP 경로 없음' };
  if (!dir || !fs.existsSync(dir)) return { ok: false, from, error: '사이트 폴더 없음' };

  const parent = path.dirname(from);
  const base = path.basename(from);
  if (/^성공$/i.test(path.basename(parent))) {
    // 이미 성공 폴더 안이면 그 파일을 재작성본으로 덮어씀
  }

  const successDir = /^성공$/i.test(path.basename(parent))
    ? parent
    : path.join(parent, '성공');
  try {
    fs.mkdirSync(successDir, { recursive: true });
  ...
  let dest = path.join(successDir, base);
  // 원본이 아직 대기열에 있고 dest가 같으면 덮어쓰기. 다른 파일이면 타임스탬프
  if (fs.existsSync(dest) && path.resolve(dest) !== path.resolve(from)) {
    // 같은 이름 기존 성공본 → 덮어써도 됨 (재배포). 유지.
  }
  ...
  try {
    ...
    // 원본 ZIP 제거 (성공 폴더가 아닌 경우만)
    if (path.resolve(from) !== path.resolve(dest) && fs.existsSync(from)) {
      try { fs.unlinkSync(from); } catch {
        try {
          const bak = path.join(successDir, `원본_${base}`);
          if (!fs.existsSync(bak)) fs.renameSync(from, bak);
          else fs.unlinkSync(from);
        } catch { /* keep original */ }
      }
    }
    return { ok: true, from, path: dest, rewritten: true };
  } catch (e) {
    // 폴백: 기존처럼 원본만 이동
    const mv = moveZipToSuccessFolder(from);
    if (mv.ok) return { ...mv, rewritten: false, error: e.message };
    return { ok: false, from, error: e.message || '재압축 실패' };
  }
}
