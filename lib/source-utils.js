...
export function rewriteSitePublicUrls(siteDir, siteRootUrl, sendLog = null) {
  ...
  const rewriteHtml = (html, fileRel) => {
    ...
    // 상대경로 기준 페이지 canonical이 비었으면 파일 경로로 보정하지 않음(기존 유지)
    void fileRel;
    return out;
  };
  ...
  // sitemap.xml: loc 호스트를 배포 주소로 강제 (경로 유지)
  let sitemap = false;
    ...
    try {
      ...
      xml = xml.replace(/<loc>\s*([^<\s]+)\s*<\/loc>/gi, (_, loc) => {
        ...
        try {
          ...
          return `<loc>${toAbs(u.pathname + (u.search || ''))}</loc>`;
        } catch {
  ...
  try {
    let rb = fs.existsSync(rbPath) ? fs.readFileSync(rbPath, 'utf8') : 'User-agent: *\nAllow: /\n';
