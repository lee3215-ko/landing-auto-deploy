...
export async function solveCaptcha(page, folder, apiKey, options = {}) {
  ...
  let tally2 = new Map();
  if (refreshCompare) {
    const prevKey = captchaKey || captchaKeyFromSrc(imgSrc) || imgSrc;
    log('새로고침 1회 → 동일 문자열·다른 배경으로 재인식');
    const refreshed = await refreshCaptchaImage(page);
    if (refreshed) {
      await new Promise((r) => setTimeout(r, 900));
      const after = await waitCaptchaKeyChanged(captchaFrame, prevKey, 8000);
      const keyChanged = !!(prevKey && after.key && after.key !== prevKey);
      if (!keyChanged) {
        // key가 같으면 다른 이미지로 OCR해도 합산이 오염됨 (로그: 6CU9 + N2YS 합쳐 오답 제출)
        log(`새로고침 후에도 캡챠 key 동일(${(after.key || prevKey || '').slice(0, 12)}…) — 비교 OCR 생략, 1차 표만 사용`);
      } else {
        // DOM 재추출 (input id 바뀌는 경우 있음)
