...
export async function solveCaptcha(page, folder, apiKey, options = {}) {
  ...
  action(`solveCaptcha 시작 (단계 ${esc.level}: YesCaptcha=${yesKey ? 'ON' : 'OFF'}, 투표 ${esc.voteCount}회, 변형 ${esc.variantCount}종${hints.trainedAt ? ', 학습모델ON' : ''})`);
  ...
  // ── 1) YesCaptcha ImageToText 우선 (재시도 + 이미지 변형으로 실패율↓) ──
  if (yesKey && !question) {
    const { solveImageToTextYesCaptcha } = await import('./yescaptcha.js');
    const yesBodies = [{ label: 'raw', b64: captured.buf.toString('base64') }];
    try {
      if (sharp) {
        const meta = await sharp(captured.buf).metadata();
        const targetW = Math.min(640, Math.max(240, (meta.width || 160) * 2));
        const up = await sharp(captured.buf)
          .resize({ width: targetW, kernel: 'lanczos3' })
          .sharpen({ sigma: 1.1 })
          .normalize()
          .png()
          .toBuffer();
        yesBodies.push({ label: 'upscale', b64: up.toString('base64') });
        const gray = await sharp(captured.buf).greyscale().linear(1.3, -12).png().toBuffer();
        yesBodies.push({ label: 'gray', b64: gray.toString('base64') });
      }
    } catch { /* keep raw only */ }

    const retries = Math.min(hints.yesCaptchaRetries, yesBodies.length);
    for (let yi = 0; yi < retries; yi++) {
      const body = yesBodies[yi % yesBodies.length];
      try {
        log(`YesCaptcha 시도 ${yi + 1}/${retries} (${body.label})`);
        const raw = await solveImageToTextYesCaptcha({
          ...
          minLength: 4,
          maxLength: 8,
          timeoutMs: yi === 0 ? hints.yesTimeoutMs : Math.min(hints.yesTimeoutMs + 15000, 75000),
        });
        const c = cleanAnswer(raw, false);
        if (c && !(hints.avoidAnswers || []).includes(c)) {
          log(`YesCaptcha 채택 "${c}"`);
          const alternatives = expandCaptchaCaseVariants(c)
            .concat(expandWithCharHints(c))
        ...
        if (c) {
          log(`YesCaptcha "${c}" — 학습 회피 목록에 있어 재시도`);
          votes.push(c);
        }
      } catch (e) {
        log(`YesCaptcha 실패(${body.label}): ${e.message}`);
        logCaptchaFailure({
          ...failBase,
          solver: 'yescaptcha',
          reason: e.message || 'yescaptcha_error',
          answers: [],
          meta: { variant: body.label, try: yi + 1 },
        });
      }
    }
    if (votes.length) {
      // avoid에만 걸린 경우라도 후보로 넘김
      const c = filterLearnedAnswers(votes)[0] || votes[0];
      if (c) {
        return packResult({
          answer: c,
          alternatives: votes,
    ...
    log('YesCaptcha 미해결 → GPT 폴백');
  }

  if (!apiKey) {
    log('OpenAI API key 없음 (YesCaptcha도 실패)');
    logCaptchaFailure({
      ...failBase, solver: 'yescaptcha', reason: 'no_openai_fallback', answers: votes,
    });
    return '';
  }
  ...
  try {
    ...
    if (!votes.length) {
      log('유효한 OCR 결과 없음');
      logCaptchaFailure({
        ...failBase, solver: 'openai', reason: 'empty_ocr', answers: [],
      });
      return '';
    }
    ...
    for (const a of alternatives) {
      ...
      for (const v of expandWithCharHints(a)) {
        if (isPlausibleCaptchaCode(v, false) && !expanded.includes(v)) expanded.push(v);
      }
    }
    alternatives = filterLearnedAnswers(expanded).slice(0, Math.max(esc.topCandidates, 4));
    const answer = alternatives[0];
    if (!answer) {
      logCaptchaFailure({
        ...failBase, solver: 'openai', reason: 'no_plausible', answers: expanded,
      });
      return '';
    }
    ...
    logCaptchaFailure({
      ...failBase, solver: 'openai', reason: e.message || 'api_error', answers: votes,
    });
    return '';
  }
}
