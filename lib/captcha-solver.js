...
import { callOpenAiVision } from './openai-vision.js';
...
/** 1차·2차(새로고침 후) 표를 합산 — 양쪽 공통 답을 최우선, 접두어 충돌 시 긴 쪽 우대 */
function mergeRefreshCompareTallies(tally1, tally2) {
  ...
  ranked.sort((a, b) => {
    if (a.both !== b.both) return b.both - a.both;
    // DTU7 vs DTUZ7 처럼 한쪽이 접두어면 짧은 쪽(1차 몰표)을 이기지 못하게
    const aAns = a.answer;
    const bAns = b.answer;
    const prefixPair = (aAns.length !== bAns.length)
      && (aAns.startsWith(bAns) || bAns.startsWith(aAns));
    if (prefixPair) {
      if (aAns.length !== bAns.length) return bAns.length - aAns.length;
    }
    if (a.count !== b.count) return b.count - a.count;
    // 2차(다른 배경)에서 나온 표를 약간 우대
 ...
 * @param {string} [options.yesCaptchaClientKey] YesCaptcha ImageToText 1차 시도 (실패 시 GPT)
...
export async function solveCaptcha(page, folder, apiKey, options = {}) {
  ...
  // 글자캡챠: YesCaptcha 1차 → 실패 시 GPT + 새로고침 비교.
  ...
  action(`solveCaptcha 시작 (단계 ${esc.level}: Yes우선+GPT폴백, 새로고침비교=${refreshCompare ? 'ON' : 'OFF'}, 투표 ${esc.voteCount}회, 변형 ${esc.variantCount}종${hints.trainedAt ? ', 학습모델ON' : ''})`);
  ...
  // ── YesCaptcha ImageToText 1회 (성공 시 GPT 생략) ──
  if (yesKey && !question) {
    try {
      const {
      ...
      if (isYesCaptchaOutOfCredit()) {
        log('YesCaptcha 잔액 없음 — GPT OCR로 진행');
      } else {
        log('YesCaptcha ImageToText 1차 시도…');
        const raw = await solveImageToTextYesCaptcha({
          clientKey: yesKey,
          bodyBase64: captured.buf.toString('base64'),
          sendLog: (m) => log(String(m)),
          caseSensitive: true,
          minLength: 4,
          maxLength: 8,
        });
        const cleaned = cleanAnswer(raw, false);
        if (cleaned && isPlausibleCaptchaCode(cleaned, false)) {
          log(`✅ YesCaptcha 채택: "${cleaned}" (GPT 생략)`);
          return packResult({
            answer: cleaned,
            alternatives: expandWithCharHints(cleaned),
            inputId,
            inputSelector,
            captchaFrameIndex,
            esc,
        ...
        log(`⚠ YesCaptcha 결과 부적합: "${raw || ''}" → GPT로 폴백`);
      }
    } catch (e) {
      log(`⚠ YesCaptcha 실패: ${e.message} → GPT로 폴백`);
    }
  }
  ...
  // ── 글자 캡챠: Yes 실패 후 GPT. 1차 인식 → 새로고침 1회 → 2차 인식 → 공통 답 우선 채택 ──
  if (!apiKey) { log('글자 캡챠 GPT 경로인데 OpenAI 키 없음'); return ''; }
  ...
  // 양쪽 일치한 답만 우선, 대안은 최대 5개 (오답 시 같은 이미지에서 순서대로 시도)
