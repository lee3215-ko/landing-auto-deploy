...
import { ensureNoNaverProtection } from './naver-protection-guard.js';
import fs from 'fs';
...
function attachAcceptDialogs(page, label = '확인 창', { allowEnterFallback = true } = {}) {
  ...
  const st = attachSafeDialogHandler(page, {
    ...
    // 자동 등록/수집 중에는 삭제 확인을 막아 실수 삭제 방지
    protectDelete: true,
  });
  return {
    ...
    detach() {
      // 라벨/로그 함수만 원복 — 핸들러는 페이지당 1개 유지, 삭제 보호 해제(수동 삭제 가능)
      if (st) {
        st.protectDelete = false;
        if (prevLog) st.log = prevLog;
        else st.log = (m) => log(m);
      }
    },
  };
...
async function checkBoardOwnershipPending(page, siteUrl) {
  ...
  try {
    await ensureAdvisorPage(page, BOARD, { settleMs: 1200, label: '보드' }).catch(() => {});
    await sleep(2000);
  } catch { /* ignore */ }
...
/** http/https 스킴 변형 URL 목록 */
function schemeVariantUrls(siteUrl = '') {
  const raw = String(siteUrl || '').trim();
  if (!raw) return [];
  const withSlash = raw.endsWith('/') ? raw : `${raw}/`;
  const noSlash = withSlash.replace(/\/$/, '');
  const bases = [withSlash, noSlash];
  const alts = [];
  for (const b of bases) {
    if (/^https:/i.test(b)) alts.push(b.replace(/^https:/i, 'http:'));
    else if (/^http:/i.test(b)) alts.push(b.replace(/^http:/i, 'https:'));
  }
  const stamped = [`${withSlash}?naver_verify=${Date.now()}`];
  return [...bases, ...alts, ...stamped].filter((u, i, arr) => u && arr.indexOf(u) === i);
}

/**
 * 라이브 사이트에 네이버 인증 메타가 반영될 때까지 대기
 * @returns {{ ok:boolean, url?:string, accessFail?:boolean, sawHtml?:boolean, reason?:string }}
 */
export async function waitUntilMetaLive(siteUrl, content, { maxWaitMs = 45000, intervalMs = 3000 } = {}) {
  ...
  let htmlOkCount = 0;
  const candidates = schemeVariantUrls(siteUrl);
      ...
      for (const ua of [
        ...
        try {
          ...
          if (html.includes(content) || new RegExp(`naver-site-verification[^>]{0,80}${content}`, 'i').test(html)) {
            ...
            return { ok: true, url, accessFail: false, sawHtml: true };
          }
        } catch (e) {
          accessFailCount += 1;
          log(`     ⚠ 라이브 접근 실패 (${ua.split('/')[0]} · ${url.replace(/https?:\/\//i, '').slice(0, 40)}): ${e.message}`);
        }
      }
    }
  ...
  const accessFail = htmlOkCount === 0 && accessFailCount > 0;
  return {
    ok: false,
    accessFail,
    sawHtml: htmlOkCount > 0,
    reason: accessFail
      ? 'access_fail: 사이트 HTTP 응답 없음(SSL/DNS/차단 가능)'
      : 'meta_missing: HTML은 읽히나 인증 메타 미포함',
  };
}

function isMetaLiveOk(result) {
...
export async function loginNaverForAdvisor(page, naverAccount, {
  ...
  const dlg = attachSafeDialogHandler(page, { log: (m) => log(m), protectDelete: true });
...
/**
 * 서치어드바이저 URL이 같은 화면인지 비교 (pathname + site 쿼리).
 * 보호조치 완화를 위해 불필요한 goto 스킵에 사용.
 */
export function isSameAdvisorPage(currentUrl, targetUrl) {
  try {
    const a = new URL(String(currentUrl || ''));
    const b = new URL(String(targetUrl || ''));
    if (!/searchadvisor\.naver\.com$/i.test(a.hostname)) return false;
    if (a.hostname !== b.hostname) return false;
    const ap = a.pathname.replace(/\/+$/, '') || '/';
    const bp = b.pathname.replace(/\/+$/, '') || '/';
    if (ap.toLowerCase() !== bp.toLowerCase()) return false;
    const as = a.searchParams.get('site') || '';
    const bs = b.searchParams.get('site') || '';
    if (bs && as && decodeURIComponent(as) !== decodeURIComponent(bs)) return false;
...
/** 이미 목표 페이지면 goto 생략 */
export async function ensureAdvisorPage(page, targetUrl, {
  ...
  const dest = String(targetUrl || '').trim();
  if (!dest) return { navigated: false, url: '' };
  let cur = '';
  try { cur = page.url() || ''; } catch { cur = ''; }
  if (!force && isSameAdvisorPage(cur, dest)) {
    if (label) log(`     ↪ 이미 ${label} — 이동 생략`);
    return { navigated: false, url: cur };
  }
  if (label) log(`     → ${label} 이동`);
  await page.goto(dest, { waitUntil, timeout });
  if (settleMs > 0) await sleep(settleMs);
  try { cur = page.url() || ''; } catch { cur = dest; }
  return { navigated: true, url: cur };
}
...
export async function selectFastCrawlMode(page, siteUrl, outputFolder = './output') {
  ...
  const optionUrl = `https://searchadvisor.naver.com/console/site/option?site=${encodedSite}`;
  await ensureAdvisorPage(page, optionUrl, { settleMs: 2000, label: '설정(빠르게)' });
...
export async function requestRobotsTxtCollect(page, siteUrl, outputFolder = './output') {
  ...
  try {
    try {
      await ensureAdvisorPage(page, robotsPage, { settleMs: 800, label: 'robots.txt' });
    } catch (e) {
    ...
    dialogs.detach();
    try {
      const st = getDialogState(page);
      if (st) st.protectDelete = false;
    } catch { /* ignore */ }
  }
}
 ...
 *  - 같은 폼 URL에 이미 있으면 goto 생략 (URL마다 재접속 → 보호조치 유발 방지)
 *  - forceNavigate=true 이면 항상 재진입
 */
export async function submitAdvisorTextRequest(page, formPageUrl, value, outputFolder = './output', {
  screenshotTag = 'form',
  forceNavigate = false,
} = {}) {
  ...
  const tryOnce = async (force) => {
    ...
    // OAuth에 있으면 먼저 탈출 (목표 폼으로)
    if (/searchadvisor\.naver\.com\/auth\//i.test(cur)) {
      await escapeAdvisorOauthCallback(page, formPageUrl);
      force = true;
    }

    const alreadyOnForm = !force && isSameAdvisorPage(cur, formPageUrl);
    if (alreadyOnForm) {
      // 폼 입력칸이 살아있는지 짧게 확인 — 없으면 재진입
      const quick = await waitForAdvisorFormInput(page, 3500).catch(() => null);
      if (quick?.ok) {
        log('     ↪ 수집 폼 재사용 (페이지 이동 없음)');
        return fillAndSubmit();
      }
      log('     ⚠ 폼 입력칸 없음 — 같은 URL로 재진입');
    }

    await ensureAdvisorPage(page, formPageUrl, {
      settleMs: alreadyOnForm ? 1200 : 1500,
      force: true,
      label: '수집 폼',
    });

    if (await escapeAdvisorOauthCallback(page, formPageUrl)) {
      await ensureAdvisorPage(page, formPageUrl, { settleMs: 1200, force: true, label: '수집 폼(재)' });
    }
    ...
    } catch { /* waitForAdvisorFormInput 에서 재확인 */ }

    try {
      cur = page.url() || '';
      if (/searchadvisor\.naver\.com\/auth\//i.test(cur)) {
        await escapeAdvisorOauthCallback(page, formPageUrl);
        await ensureAdvisorPage(page, formPageUrl, { settleMs: 1500, force: true, label: '수집 폼' });
      }
    } catch { /* ignore */ }
  ...
  try {
    return await tryOnce(forceNavigate);
  } catch (e) {
    const msg = e?.message || '';
...
export async function registerNaverSites({
  ...
  /** 메타 라이브 대기 상한(ms). 닷홈은 DNS/캐시 때문에 더 길게 */
  metaLiveMaxWaitMs = 0,
  ...
  try {
    ...
    for (const site of sites) {
      ...
      await ensureNoNaverProtection(page, { log, source: 'naver-register' });
      log(`\n========================================`);
      log(`📝 [${site.name}]: ${site.url}`);
      log(`========================================`);
      try {
        // (A) console/board 이동
        log('  (A) console/board 확인...');
        await ensureAdvisorPage(page, BOARD, { settleMs: 1200, label: '보드' });
        ...
        if (redeployCallback) {
          ...
          const waitMs = metaLiveMaxWaitMs > 0
            ? metaLiveMaxWaitMs
            : (extraRedeployOnMiss ? 60000 : 90000);
          let liveMeta = await waitUntilMetaLive(site.url, content, { maxWaitMs: waitMs, intervalMs: 4000 });
          if (!isMetaLiveOk(liveMeta) && extraRedeployOnMiss) {
            log(`     ⚠ 라이브 메타 미확인 (${liveMeta?.reason || 'unknown'}) — 재배포 1회 추가 시도`);
            try {
              await redeployCallback(site, metaTag);
            } catch (redeployErr) {
              const msg = redeployErr?.message || String(redeployErr);
            ...
            liveMeta = await waitUntilMetaLive(site.url, content, { maxWaitMs: 45000, intervalMs: 4000 });
            if (!isMetaLiveOk(liveMeta)) {
              log(`     ⚠ 라이브 메타 여전히 미확인 (${liveMeta?.reason || ''}) — 소유확인 전 추가 대기`);
            }
          } else if (!isMetaLiveOk(liveMeta)) {
            log(`     ⚠ 라이브 메타 미확인 (${liveMeta?.reason || ''}) — 추가 재배포 생략, 소유확인 전 대기 강화`);
          }
          // 소유확인 직전: 미확인이면 짧게 한 번 더 (네이버봇 반영 여유)
          if (!isMetaLiveOk(liveMeta)) {
            log('     ⏳ 소유확인 전 메타/접근 재확인 18초…');
            await sleep(8000);
            liveMeta = await waitUntilMetaLive(site.url, content, { maxWaitMs: 45000, intervalMs: 3000 });
            if (!isMetaLiveOk(liveMeta)) {
              log(`     ⚠ 최종 라이브 미확인 — ${liveMeta?.reason || 'unknown'} (소유확인은 계속, 실패 시 유형 분리)`);
            }
          }
          liveMetaState = liveMeta || liveMetaState;
        } else {
        ...
        /** @type {''|'captcha'|'meta_missing'|'access_fail'} */
        let ownershipFailKind = '';
        ...
        while (captchaAttempts < maxCaptchaAttempts) {
          ...
          if (answer && answer.length >= 1) {
            ...
            if (/메타\s*태그|찾을\s*수\s*없|호스팅/.test(msg)) {
              ownershipFailKind = 'meta_missing';
              log('     ⚠ 메타태그 미검출 팝업 — 라이브 재확인 후 소유확인 재시도');
              log(`     기대 content: ${content}`);
              const liveOkRaw = await waitUntilMetaLive(site.url, content, {
                maxWaitMs: 45000,
                intervalMs: 3000,
              });
              const liveOk = isMetaLiveOk(liveOkRaw);
              if (liveOkRaw?.accessFail) ownershipFailKind = 'access_fail';
              ...
              if (shouldForceRedeploy) {
                try {
                  ...
                  const after = await waitUntilMetaLive(site.url, content, { maxWaitMs: 60000, intervalMs: 4000 });
                  if (isMetaLiveOk(after)) liveMetaState = after;
                  else if (after?.accessFail) ownershipFailKind = 'access_fail';
            ...
            if (/실패|보안절차|자동등록|자동입력|잘못\s*입력/.test(msg)) {
              if (ownershipFailKind !== 'meta_missing' && ownershipFailKind !== 'access_fail') {
                ownershipFailKind = 'captcha';
              }
              const nextAlt = candidates[ci + 1];
              let stillSame = false;
              try {
                stillSame = await captchaImageStillFresh(
                  page,
                  captchaResult?.captchaKey || '',
                  captchaResult?.imgSrc || '',
                );
              } catch { stillSame = false; }
              if (nextAlt && stillSame) {
                log(`     ❌ 오답 "${answer}" — 같은 이미지로 대안 시도: ${nextAlt}`);
                await sleep(500);
                continue;
              }
              log(`     ❌ 캡챠/검증 실패 (팝업) — ${nextAlt ? '이미지 변경됨·' : ''}대안 중단, 새로고침`);
              await sleep(600);
              break;
            }
            ...
            if (hasFailText) {
              ...
              if (nextAlt && stillSame) {
                log(`     ❌ 실패(본문) "${answer}" — 대안 시도: ${nextAlt}`);
                await sleep(500);
                continue;
              }
              log('     ❌ 실패(fallback) — 대안 중단, 이미지 새로고침');
              break;
            }
        ...
        if (isVerifyUrl(finalUrl) && !verifyUrlMatchesSite(finalUrl, site.url)) {
          ...
          if (/등록\s*완료|성공|확인\s*완료|소유\s*확인\s*완료/.test(body)) { log(`  ✅ 등록 성공`); status = 'success'; }
            ...
            ownershipFailKind === 'meta_missing'
            || /메타\s*태그|찾을\s*수\s*없|호스팅/.test(lastPop)
            || /메타\s*태그|찾을\s*수\s*없/.test(body)
          ) {
            log(`  ⚠ 메타미검출 (캡챠 아님)`);
            status = 'meta_missing';
            resultError = 'meta_missing: 네이버가 사이트에서 메타태그를 찾지 못함 — http/배포/접근 확인 후 「수동캡챠」';
          } else if (
            ownershipFailKind === 'access_fail'
            || liveMetaState?.accessFail
            || /접근\s*실패|응답을\s*받지|Status\s*0/i.test(lastPop + body)
          ) {
            log(`  ⚠ 사이트 접근 실패 (캡챠 아님)`);
            status = 'access_fail';
            resultError = 'access_fail: 네이버/로컬이 사이트에 접근 실패 — http·DNS·방화벽 확인';
          } else if (/캡챠|보안|자동|로봇/.test(body) || captchaAttempts > 0 || ownershipFailKind === 'captcha') {
            log(`  ⚠ 캡챠/보안 문제`); status = 'captcha';
          } else if (!isMetaLiveOk(liveMetaState) && liveMetaState?.reason) {
            // 라이브 메타도 못 봤는데 팝업도 모호 → 접근/메타 쪽으로
            status = liveMetaState.accessFail ? 'access_fail' : 'meta_missing';
            resultError = liveMetaState.reason;
            log(`  ⚠ ${status} (라이브 미확인 기반)`);
          } else { log(`  ⚠ 상태 미확인`); }
        }
        ...
        if (status === 'captcha') {
          ...
          if (!resultError || /메타\s*태그|찾을\s*수\s*없|호스팅|접근\s*실패/i.test(resultError)) {
            resultError = content
        ...
        } else if (status === 'meta_missing' || status === 'access_fail') {
          consecutiveCaptchaFails = 0;
          if (!resultError) {
            resultError = status === 'access_fail'
              ? 'access_fail: 사이트 접근 실패 — http·DNS 확인 후 「수동캡챠」'
              : 'meta_missing: 메타태그 미검출 — 배포/스킴 확인 후 「수동캡챠」';
          }
          log(`  ℹ ${status} — 캡챠 오답과 별도 처리`);
        ...
        results.push({
          ...
          failKind: status === 'captcha' || status === 'meta_missing' || status === 'access_fail' ? status : ownershipFailKind || undefined,
          metaContent: typeof content === 'string' ? content : '',
          metaDeployed: !!(typeof content === 'string' && content),
          liveMetaOk: isMetaLiveOk(liveMetaState),
          naverAccountId: naverAccount?.id || '',
          registeredAt: new Date().toISOString(),
        });
      ...
      await sleep(1500);
      try {
        await ensureAdvisorPage(page, BOARD, { settleMs: 800, label: '보드' });
      } catch {}
    }

    // 자동 OCR 재시도는 성공률 낮고 HTML라디오/쿨다운 낭비 → 배포결과 「수동캡챠」로 처리
    const captchaFails = results.filter((r) => r.status === 'captcha');
    const metaFails = results.filter((r) => r.status === 'meta_missing');
    const accessFails = results.filter((r) => r.status === 'access_fail');
    if (captchaFails.length && !abortRemaining) {
      log(`\nℹ 캡챠 실패 ${captchaFails.length}개 — 자동 재시도 생략`);
      log('   → 배포 결과 탭에서 「수동캡챠」를 누르면 캡챠 화면으로 연결됩니다.');
    }
    if (metaFails.length) {
      log(`\nℹ 메타미검출 ${metaFails.length}개 — 캡챠 오답과 별개 (http/배포 확인 후 수동캡챠)`);
    }
    if (accessFails.length) {
      log(`\nℹ 접근실패 ${accessFails.length}개 — 사이트 HTTP 응답 없음`);
    }

  } catch (e) {
    log(`❌ 전체 오류: ${e.message}`);
  } finally {
    // 등록 끝나면 보드에서 사이트 선택·삭제가 다시 되도록 보호 해제
    try {
      const st = getDialogState(page);
      if (st) st.protectDelete = false;
      else if (page && !page.isClosed?.()) attachSafeDialogHandler(page, { protectDelete: false });
    } catch { /* ignore */ }
