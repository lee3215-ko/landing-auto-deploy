...
import { attachSafeDialogHandler, waitForDialogAccept } from './dialog-guard.js';
...
function attachAcceptDialogs(page, label = '확인 창') {
  ...
  return {
    ...
    async waitForDialog(timeoutMs = 4000) {
      return waitForDialogAccept(page, timeoutMs);
    },
    detach() {
      /* 단일 핸들러 유지 — off 하지 않음 */
    },
...
export async function registerNaverSites({
  ...
  try {
      ...
      try {
        ...
        while (captchaAttempts < maxCaptchaAttempts) {
          ...
          if (!candidates.length) {
            ...
            const cool = Math.min(3000 + captchaAttempts * 2500, 15000);
          ...
          await refreshCaptchaImage(page);
          const cool = Math.min(4000 + captchaAttempts * 2000, 18000);
          log(`     ⏳ 캡챠 쿨다운 ${Math.round(cool / 1000)}초…`);
          await sleep(cool);
          await page.evaluate(() => {
        ...
        if (status === 'captcha') {
          ...
          if (consecutiveCaptchaFails >= 2) {
            const cool = Math.min(20000 + consecutiveCaptchaFails * 10000, 90000);
    ...
    // 캡챠 실패 사이트 1회 재시도 (쿨다운 후)
    const captchaRetry = results.filter((r) => r.status === 'captcha');
    if (captchaRetry.length && !abortRemaining) {
      log(`\n🔄 캡챠 실패 ${captchaRetry.length}개 재시도…`);
      await sleep(25000);
      for (const prev of captchaRetry) {
        const site = sites.find((s) => s.url === prev.url);
        if (!site) continue;
        if (dlg) {
          dlg.ownershipDone = false;
          dlg.lastMsg = '';
        }
        log(`\n━━━ 캡챠 재시도: ${site.name || site.url} ━━━`);
        try {
          await openVerifyPage(page, site.url);
          await sleep(2000);
          let clicked = await page.evaluate(() => {
            for (const el of document.querySelectorAll('button, a, div[role="button"]')) {
              const t = (el.textContent || '').trim();
              if (/소유확인|소유 확인/.test(t)) { el.click(); return t.slice(0, 30); }
            }
            return '';
          }).catch(() => '');
          if (clicked) log(`     👆 "${clicked}"`);
          await sleep(8000);

          let ok = ownershipDoneByDialog() || /\/console\/site\//i.test(page.url());
          if (!ok) {
            const has = await detectCaptcha(page);
            if (has) {
              const cap = await solveCaptcha(page, site.folder, openaiApiKey, { attemptLevel: 3 });
              const ans = (cap?.alternatives || [cap?.answer]).filter(Boolean)
                .find((a) => isPlausibleCaptchaCode(a, false));
              if (ans) {
                // 간단 입력 재시도: solveCaptcha 경로와 별도로 한 번만
                log(`     캡챠 재시도 코드: ${ans}`);
                await page.evaluate((code) => {
                  const sels = ['input#captcha', 'input[name="captcha"]', 'input[data-detect="code"]', '.captcha_wrap input[type="text"]'];
                  for (const s of sels) {
                  ...
                  for (const btn of document.querySelectorAll('button, a, div[role="button"]')) {
                    if (/^(확\s*인|확인)$/.test((btn.textContent || '').trim())) {
                      btn.click();
                      return;
                    }
                  }
            ...
            ok = ownershipDoneByDialog()
              || /완료되었습니다|소유\s*확인이\s*완료/i.test(lastDialogMsg())
              || /\/console\/site\//i.test(page.url());
          }
          if (ok) {
            prev.status = 'success';
            delete prev.error;
            log(`  ✅ 캡챠 재시도 성공: ${site.url}`);
            if (!skipIndexing) {
              try { await requestNaverIndexing(page, site.url, site.folder); } catch { /* ignore */ }
            }
          } else {
            log(`  ⚠ 캡챠 재시도 실패: ${site.url}`);
          }
        } catch (re) {
          log(`  ⚠ 캡챠 재시도 오류: ${re.message}`);
        }
        await sleep(4000);
        try {
          await page.goto(BOARD, { waitUntil: 'domcontentloaded', timeout: 20000 });
