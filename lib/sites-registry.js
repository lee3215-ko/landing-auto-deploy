...
export function mergeLegacySources(existing, { results = [], dothomeAccounts = [], cloudflareSites = [] } = {}) {
  ...
  for (const r of results || []) {
    ...
    // 넷리파이 생성(kkang) + 설정 전체 실행(settings-pipeline) + netlify.app URL
    const fromSettings = source.includes('settings') || source.includes('pipeline');
    const fromKkang = source.includes('kkang');
    const looksNetlify = /\.netlify\.app/i.test(url);
    if (!fromKkang && !fromSettings && !looksNetlify) continue;
    ...
    list = upsertSite(list, {
      ...
      detail: {
        ...
        from: fromSettings ? 'settings-pipeline' : (fromKkang ? 'kkang' : 'results'),
      },
    });
  }
