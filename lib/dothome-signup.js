...
async function solveDothomeCaptcha(page, folder, apiKey, sendLog, {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    // YesCaptcha 키가 있으면 우선 사용 (OpenAI 크레딧 소진 시에도 동작)
    if (yesKey) {
      try {
        const answer = await solveDothomeCaptchaYesCaptcha(buf, yesKey, sendLog);
        if (answer && isPlausibleCaptchaCode(answer)) {
          relay(sendLog, `✅ 보안문자 채택(YesCaptcha): ${answer}`);
          await setInputValue(page, '#captcha', answer);
          return answer;
        }
        if (answer) {
          relay(sendLog, `⚠ YesCaptcha 결과 형식 이상: "${answer}" — OpenAI/재시도`);
        }
      } catch (e) {
    ...
    if (!apiKey) {
      if (round === maxRounds - 1) {
        throw new Error(
          yesKey
            ? '보안문자 인식 실패 (YesCaptcha). OpenAI 키를 추가하거나 YesCaptcha 잔액을 확인하세요.'
            : '보안문자 인식 실패. 설정 탭에 OpenAI 또는 YesCaptcha 키가 필요합니다.',
        );
      }
    ...
    let result;
    try {
      result = await ocrCaptchaConsensus(apiKey, buf, sendLog, level);
    } catch (e) {
      if (e instanceof OpenAiCreditsError || isOpenAiCreditsError(e)) {
        // OpenAI 소진 + YesCaptcha 없으면 즉시 중단 (27회 반복 방지)
