...
import { callOpenAiVision, isOpenAiCreditsError } from './openai-vision.js';
...
function relay(sendLog, msg) {
  // sendLog가 있으면 UI로만 1회. sharedLog까지 같이 치면 setLogger 경유 시 줄이 2배로 쌓임.
...
async function captureCaptchaBuffer(page, folder, tag = 'captcha') {
  ...
  // 중요: /common/chsignup/chsignup.php 를 다시 fetch 하면
  // 서버 세션 보안문자가 바뀌어 화면(#capt_img)과 불일치한다.
  // 화면에 보이는 요소 스크린샷만 사용해 OCR한다.
  await page.waitForFunction(() => {
    const img = document.querySelector('#capt_img');
    return !!(img && img.complete && (img.naturalWidth || 0) > 8);
  }, { timeout: 15000 }).catch(() => {});
  ...
  if (!imgHandle) throw new Error('보안문자 이미지(#capt_img) 없음');
  await imgHandle.screenshot({ path: shotPath, type: 'png' });
  const buf = fs.readFileSync(shotPath);
  if (!buf || buf.length < 80) throw new Error('보안문자 이미지를 캡처하지 못했습니다.');

  return { buf, shotPath, srcInfo };
}

/** YesCaptcha용: 격자 배경 완화 + 업스케일 (threshold는 글자 뭉개서 사용 안 함) */
async function prepareCaptchaForYes(buf, folder, tag) {
  const out = [];
  const push = (b, label) => {
    try {
      fs.writeFileSync(path.join(folder, `${tag}_yes_${label}.png`), b);
    } catch { /* ignore */ }
    out.push({ buf: b, label });
  };
  ...
  try {
    ...
    if (meta.format === 'gif' || (meta.pages && meta.pages > 1)) {
      base = await sharp(raw, { pages: 1 }).png().toBuffer();
    }
    const w = Math.max(meta.width || 80, 40);
    // 닷홈 격자 캡챠는 너무 키우면 격자선이 글자로 오인됨 → 2.5~3배
    const targetW = Math.max(Math.round(w * 3), 200);

    // 1) 컬러 업스케일 (격자+글자 대비 유지 — 1순위)
    const upColor = await sharp(base)
...
async function refreshDothomeCaptcha(page) {
  const clicked = await page.evaluate(() => {
    ...
    // 옆에 있는 새로고침 아이콘(캡챠 이미지 다음 형제)
    const img = document.querySelector('#capt_img');
    if (img) {
      const sib = img.nextElementSibling;
      if (sib) { sib.click(); return 'sibling'; }
      img.src = `/common/chsignup/chsignup.php?${Date.now()}`;
      return 'src-bust';
    }
    return '';
  });
  if (!clicked) {
    try { await page.click('#reload_capt_btn'); } catch { /* ignore */ }
  }
  await sleep(1400);
  await page.waitForFunction(() => {
    const img = document.querySelector('#capt_img');
...
async function ocrCaptchaConsensus(apiKey, inputBuf, sendLog, level = 0) {
  const voteCount = Math.min(3 + level, 5);
  const variantCount = Math.min(2 + level, 3);
  ...
  const temps = [0, 0.05, 0.1, 0.15, 0.2].slice(0, voteCount);

  relay(sendLog, `OCR 합의 L${level}: 변형 ${variants.length}종 · 투표 ${voteCount}회`);
...
async function solveDothomeCaptchaYesCaptcha(buf, yesKey, sendLog, folder = '', tag = 'captcha', preferM1 = false) {
  const {
    solveImageToTextYesCaptcha,
    isYesCaptchaBalanceError,
    isYesCaptchaOutOfCredit,
  } = await import('./yescaptcha.js');

  const variants = await prepareCaptchaForYes(buf, folder || process.cwd(), tag);
  let lastErr = '';

  const tryOne = async (variant, taskType) => {
    relay(sendLog, `YesCaptcha ${taskType} (${variant.label})…`);
    const text = await solveImageToTextYesCaptcha({
      clientKey: yesKey,
      bodyBase64: variant.buf.toString('base64'),
      sendLog,
    ...
    const cleaned = cleanCaptchaAnswer(text) || String(text || '').replace(/\s+/g, '').replace(/[^A-Za-z0-9]/g, '');
    if (cleaned && isPlausibleCaptchaCode(cleaned)) {
      relay(sendLog, `YesCaptcha 인식 OK (${variant.label}/${taskType}): "${cleaned}"`);
      return cleaned;
    }
    relay(sendLog, `⚠ Yes 결과 부적합 (${variant.label}): "${text || ''}"`);
    return '';
  };

  const taskOrder = preferM1
    ? ['ImageToTextTaskM1', 'ImageToTextTask']
    : ['ImageToTextTask', 'ImageToTextTaskM1'];

  const pickVariants = (taskType) => {
    if (/M1/i.test(taskType)) {
      return variants.filter((v) => v.label === 'up3' || v.label === 'shot').slice(0, 1);
    }
    // ImageToText: up3 → shot (up3g는 격자선 왜곡으로 제외)
    return variants.filter((v) => v.label === 'up3' || v.label === 'shot');
  };

  for (const taskType of taskOrder) {
    for (const variant of pickVariants(taskType)) {
      try {
        const ok = await tryOne(variant, taskType);
        if (ok) return ok;
      } catch (e) {
        lastErr = e.message || String(e);
        relay(sendLog, `⚠ Yes 실패 (${variant.label}/${taskType}): ${lastErr}`);
        if (isYesCaptchaBalanceError(e) || isYesCaptchaOutOfCredit()) throw e;
        if (/task type|type.*invalid|ERROR_TASK|UNKNOWN_TASK/i.test(lastErr) && /M1/i.test(taskType)) {
          break;
        }
      }
    }
  ...
  throw new Error(lastErr || 'YesCaptcha 닷홈 보안문자 인식 실패');
}
...
async function solveDothomeCaptcha(page, folder, apiKey, sendLog, {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    try {
      ...
      yesDisabled = !yesKey || isYesCaptchaOutOfCredit();
    } catch {
      yesDisabled = !yesKey;
    }
    // 글자 보안문자는 YesCaptcha(전용 OCR) 우선. OpenAI는 Yes 실패·잔액없음 폴백만.
    // 예전: 불일치 때 OpenAI를 무조건 채택 → 성공률이 더 떨어짐.
    const openaiOnly = !!(apiKey && yesDisabled);
    relay(sendLog, `보안문자 인식 라운드 ${round + 1}/${maxRounds}... (${openaiOnly ? 'OpenAI만' : 'YesCaptcha우선'})`);
    ...
    const runYes = async () => {
      ...
      try {
        ...
        if (isYesCaptchaOutOfCredit()) {
          if (round === 0) relay(sendLog, '⏭ YesCaptcha 잔액 없음 — 이미지 보안문자는 OpenAI만 사용');
          return '';
        }
      } catch { /* ignore */ }
      try {
        const answer = await solveDothomeCaptchaYesCaptcha(
          buf,
          yesKey,
          sendLog,
          folder,
          `captcha_r${round + 1}`,
          round >= 1, // 불일치·실패 후 라운드부터 M1 우선
        );
        ...
        try {
          const { isYesCaptchaBalanceError, isYesCaptchaOutOfCredit } = await import('./yescaptcha.js');
          if (isYesCaptchaBalanceError(e) || isYesCaptchaOutOfCredit()) {
            relay(sendLog, '⏭ 이후 YesCaptcha 호출 생략 (잔액 부족)');
          }
        } catch { /* ignore */ }
      }
      return '';
    ...
    // YesCaptcha 유효 답이면 GPT 교차검증 생략 (채택도 Yes 우선)
    if (yesAnswer) {
      relay(sendLog, `✅ 보안문자 채택(YesCaptcha): ${yesAnswer}`);
      await setInputValue(page, '#captcha', yesAnswer);
      return yesAnswer;
    }
    if (apiKey && (preferOpenAi || openaiOnly || !yesAnswer)) {
      openaiResult = await tryOpenAiCaptcha(apiKey, buf, sendLog, level, yesKey);
    }
    const oa = openaiResult.answer || '';
    if (oa) {
      relay(sendLog, `✅ 보안문자 채택(OpenAI): ${oa} (신뢰도 ${(openaiResult.confidence * 100).toFixed(0)}%)`);
      await setInputValue(page, '#captcha', oa);
      return oa;
    }
...
/** reCAPTCHA/사용자 인증이 풀린 뒤 다시 요구되는 경우 */
function isUserAuthRequiredMessage(msg = '') {
  return /사용자\s*인증을\s*해주세요|로봇이\s*아닙니다|사용자\s*인증/i.test(String(msg || ''));
}
...
function isHostingSubmitFailureMessage(msg = '') {
  ...
  if (isUserAuthRequiredMessage(m)) return true;
  ...
  if (await page.$('#staff_email_type_opt')) {
    await page.evaluate(() => {
      ...
      if (sel) {
        ...
        sel.value = opts.includes('kakao.com') ? 'kakao.com'
          : (opts.includes('직접입력') ? '직접입력' : (opts.find((v) => /kakao/i.test(v)) || 'kakao.com'));
...
async function fillSignupForm(page, { id, password, name, phone, emailLocal }, sendLog) {
  ...
  try {
    ...
    // 이메일: 로컬 + kakao.com
    relay(sendLog, `이메일: ${emailLocal}@kakao.com`);
    await page.evaluate(() => {
...
export async function runDothomeSignup({
  ...
  if (!local) throw new Error('카카오 이메일 아이디(앞부분)를 입력하세요.');
  ...
  if (!yesCaptchaClientKey) {
    ...
    try {
      const {
        resetYesCaptchaCreditState,
        getYesCaptchaBalance,
        isYesCaptchaOutOfCredit,
      } = await import('./yescaptcha.js');
      resetYesCaptchaCreditState();
      const bal = await getYesCaptchaBalance(yesCaptchaClientKey, { sendLog });
      if (bal != null && bal <= 0) {
        relay(sendLog, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        relay(sendLog, '🛑 YesCaptcha 잔액 0 — reCAPTCHA 자동 통과 불가');
        relay(sendLog, '👉 https://yescaptcha.com 에서 충전 후 다시 실행하세요.');
        relay(sendLog, '👉 충전 전까지: 보안문자(글자)는 OpenAI로 처리 · reCAPTCHA 타일은 브라우저에서 직접 클릭');
        relay(sendLog, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      } else if (bal != null) {
        relay(sendLog, `YesCaptcha 잔액: ${bal}`);
      } else if (isYesCaptchaOutOfCredit()) {
        relay(sendLog, '🛑 YesCaptcha 잔액 부족으로 표시됨 — 충전 후 재실행 권장');
      }
    } catch (e) {
      relay(sendLog, `⚠ YesCaptcha 잔액 확인 실패: ${e.message}`);
    }
  }

  // 카카오 메일: 기존 창 유지 (닷홈 탭에서 미리 로그인)
  {
    const { ensureDothomeMailSessionReady } = await import('./dothome-kakao-mail-session.js');
    ...
    relay(sendLog, `카카오 메일 세션 사용: ${mailSt.accountId}`);
  }
  ...
  relay(sendLog, `이메일: ${local}@kakao.com`);
  ...
  try {
    ...
    // 인증메일은 위에서 1회만 발송 (재발송 금지 — 만료 시에만 resendAuthCodeAfterExpire)
    // ── 카카오 메일에서 인증코드 (확인단어 통과 때 보낸 메일) ──
    const naverId = naverAccount?.id || local;
    const naverPw = naverAccount?.pw || '';
    if (!naverPw) {
      throw new Error('카카오 메일 비밀번호가 없습니다. 닷홈 탭에 메일 아이디·비밀번호를 입력하세요.');
    }

    relay(sendLog, `카카오 메일(기존 로그인 창)에서 인증코드 조회 (제목 FTP: ${ftpId})...`);
    ...
    for (let submitTry = 1; submitTry <= 4; submitTry++) {
      ...
      // 신청 재시도 중 reCAPTCHA가 풀리면 「사용자 인증을 해주세요」만 뜨고 화면이 유지됨
      if (joinDialogs.some((m) => isUserAuthRequiredMessage(m))) {
        lastSubmitError = joinDialogs.find((m) => isUserAuthRequiredMessage(m)) || '사용자 인증 필요';
        relay(sendLog, `⚠ ${lastSubmitError} — reCAPTCHA 재통과 후 재신청`);
        if (!(await passRecaptchaHumanLike(page, {
          openaiApiKey,
          yesCaptchaClientKey,
          folder,
