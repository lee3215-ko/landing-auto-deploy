...
async function solveDothomeCaptcha(page, folder, apiKey, sendLog, {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    // 글자 보안문자는 YesCaptcha(전용 OCR) 우선. OpenAI는 교차검증·폴백만.
    // 예전: 불일치 때 OpenAI를 무조건 채택 → 성공률이 더 떨어짐.
    const openaiOnly = !!(apiKey && yesDisabled);
    relay(sendLog, `보안문자 인식 라운드 ${round + 1}/${maxRounds}... (${openaiOnly ? 'OpenAI만' : 'YesCaptcha우선+교차'})`);
    ...
    if (apiKey && (preferOpenAi || !yesAnswer || openaiOnly)) {
      openaiResult = await tryOpenAiCaptcha(apiKey, buf, sendLog, level, yesKey);
    } else if (apiKey && yesAnswer) {
      openaiResult = await tryOpenAiCaptcha(apiKey, buf, sendLog, Math.min(level, 1), yesKey);
    }

    const oa = openaiResult.answer || '';
    if (yesAnswer && oa && yesAnswer.toLowerCase() === oa.toLowerCase()) {
      const pick = yesAnswer.length >= oa.length ? yesAnswer : oa;
      relay(sendLog, `✅ 보안문자 채택(교차일치): ${pick}`);
      await setInputValue(page, '#captcha', pick);
      return pick;
    }
    if (yesAnswer && oa && yesAnswer.toLowerCase() !== oa.toLowerCase()) {
      // 전용 OCR을 채택. OpenAI는 다음 불일치 라운드에서 씀
      relay(sendLog, `⚠ OCR 불일치 Yes="${yesAnswer}" / OpenAI="${oa}" → YesCaptcha 채택`);
