...
export async function runFullPipeline(config, sendLog = null, hooks = {}) {
  ...
  const validNaverAccounts = (Array.isArray(naverAccounts) ? naverAccounts : [])
    .map((a) => ({ id: String(a?.id || '').trim(), pw: String(a?.pw || '').trim() }))
    .filter((a) => a.id && a.pw);
  let currentNaverAccount = validNaverAccounts[0] || null;
  ...
  const { NAVER_SITE_SOFT_LIMIT } = await import('./naver-session.js');

  try {
    step(`네이버 계정 정책: 동일 계정 유지 → 등록 ${NAVER_SITE_SOFT_LIMIT}개 이상일 때만 다음 아이디로 전환`);
    if (currentNaverAccount) {
      step(`시작 네이버 계정: ${currentNaverAccount.id}`);
    }
    ...
    for (let batchStart = 0; batchStart < expanded.length; batchStart += BATCH_SIZE) {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        const naverAccount = currentNaverAccount;
        ...
        while (triedIndexes.size < availableIndexes.length) {
          ...
          try {
            ...
            if (/429|rate limit/i.test(msg)) {
              ...
              warn(`[${displayName}] Rate limit 429 — ${Math.round(wait / 1000)}초 대기 후 동일 토큰 재시도 (${rateLimitSameTokenTries}/3)…`);
              await new Promise((r) => setTimeout(r, wait));
              if (rateLimitSameTokenTries < 3) {
                triedIndexes.delete(usedTokenIndex);
                continue;
              }
              warn(`[${displayName}] 429 재시도 한도 — 배치 후 재시도 큐에 추가`);
      ...
      if (batchStart + BATCH_SIZE < expanded.length) {
        await checkpoint(relay);
        const rest = 20000;
        step(`=== 다음 배치 전 ${rest / 1000}초 대기 (rate limit 완화) ===`);
    ...
    if (rateLimitedRetry.length) {
      await checkpoint(relay);
      step(`=== Rate limit 재시도 큐: ${rateLimitedRetry.length}개 (60초 대기 후) ===`);
      ...
      for (const item of rateLimitedRetry) {
        ...
        const availableIndexes = pickAvailableTokenIndexes(netlifyTokens);
        if (!availableIndexes.length) {
          failedServices.push({ name: displayName, error: '429 재시도 — 유효 토큰 없음' });
          continue;
        }
        ...
        for (let attempt = 0; attempt < 3 && !done; attempt++) {
          ...
          const tokenId = typeof tokenObj === 'string' ? '' : (tokenObj?.id || '');
          try {
            warn(`[${displayName}] 429 재시도 ${attempt + 1}/3…`);
            ...
            if (/429|rate limit/i.test(msg)) {
              ...
              warn(`[${displayName}] 재시도 중 429 — ${Math.round(wait / 1000)}초 대기…`);
              await new Promise((r) => setTimeout(r, wait));
              continue;
            }
            error(`[${displayName}] 429 재시도 실패: ${msg}`);
        ...
        if (!done && !failedServices.some((f) => f.name === displayName)) {
          failedServices.push({ name: displayName, error: '429 재시도 한도 초과' });
          pushResult(results, {
            url: item.siteUrl || '',
            name: displayName,
            status: 'error',
            error: 'Netlify rate limit (429) 재시도 한도 초과',
            naverAccountId: naverAccount?.id || '',
          }, outputRoot, onResult);
        }
        await new Promise((r) => setTimeout(r, DEPLOY_GAP_MS));
