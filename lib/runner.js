...
async function prepareSiteDir({
  ...
  if (sourceType === 'zip' && sourcePath) {
    ...
    try {
      rewriteSitePublicUrls(htmlDir, String(siteUrl || '').replace(/\/$/, ''), relay);
    } catch (e) {
      warn(`[${displayName}] 사이트 URL 재작성 경고: ${e.message}`);
    }
    return htmlDir;
  }
  ...
  if (sourceType === 'folder' && sourcePath) {
    ...
    try {
      rewriteSitePublicUrls(folder, String(siteUrl || '').replace(/\/$/, ''), relay);
    } catch (e) {
      warn(`[${displayName}] 사이트 URL 재작성 경고: ${e.message}`);
    }
    return folder;
...
async function waitDeploySuccessGap(relay, { minMin, maxMin }) {
  let lo = Math.max(0, Number(minMin) || 0);
  let hi = Math.max(0, Number(maxMin) || 0);
  if (hi < lo) { const t = lo; lo = hi; hi = t; }
  if (hi <= 0) return;
  const minutes = lo === hi ? lo : (lo + Math.random() * (hi - lo));
  const totalMs = Math.round(minutes * 60_000);
  if (totalMs <= 0) return;
  const endAt = Date.now() + totalMs;
  step(`⏳ 배포 성공 — 다음까지 약 ${minutes.toFixed(1)}분 대기 (${lo}~${hi}분 랜덤)`);
  ...
  while (Date.now() < endAt) {
    ...
    if (Date.now() - lastLogAt >= 60_000) {
      ...
      relay(`⏳ 대기 남음 약 ${Math.ceil(leftSec / 60)}분…`);
    }
    const slice = Math.min(5000, Math.max(0, endAt - Date.now()));
...
export async function runFullPipeline(config, sendLog = null, hooks = {}) {
  ...
  let deployGapMinMin = Math.max(0, Number(config.deployGapMinMin) || 0);
  let deployGapMaxMin = Math.max(0, Number(config.deployGapMaxMin) || 0);
  if (deployGapMaxMin < deployGapMinMin) {
    const tmp = deployGapMinMin;
    deployGapMinMin = deployGapMaxMin;
    deployGapMaxMin = tmp;
  }
  let siteDeploySucceeded = false;
  ...
  try {
    ...
    for (let batchStart = 0; batchStart < expanded.length; batchStart += BATCH_SIZE) {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        const hasMoreSites = (batchStart + i + 1) < expanded.length;
        if (hasMoreSites && siteDeploySucceeded && deployGapMaxMin > 0) {
          await waitDeploySuccessGap(relay, { minMin: deployGapMinMin, maxMin: deployGapMaxMin });
        } else if (hasMoreSites) {
