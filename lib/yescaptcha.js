...
function normalizeImageToTextAnswer(text) {
  const t = String(text || '').trim();
  if (!t) throw new Error('solution.text 비어 있음');
  if (/sorry|can'?t|unable|i\s*am|죄송|불가/i.test(t) || t.length > 12) {
    throw new Error(`비정상 인식 결과: "${t.slice(0, 40)}"`);
  }
  return t;
}

/**
 * 이미지 캡챠(네이버 NHN·닷홈 등) → 텍스트
 * @param {string} [options.taskType] ImageToTextTask | ImageToTextTaskM1 | ImageToTextTaskOcrBase 등
...
export async function solveImageToTextYesCaptcha({
  ...
  const type = String(taskType || 'ImageToTextTask').trim() || 'ImageToTextTask';
  const syncTypes = new Set(['ImageToTextTaskOcrBase', 'ImageToTextTaskMuggle']);
  let lastErr = '';
  let taskId = '';
  let apiBase = API_NODES[0];

  for (const base of API_NODES) {
    log(`createTask ${type} @ ${base.replace('https://', '')}`);
    const created = await postJson(`${base}/createTask`, {
      clientKey,
      task: {
        type,
    ...
    // 동기 타입: createTask 응답에 solution이 바로 올 수 있음
    if (syncTypes.has(type) || created.status === 'ready' || created.solution?.text) {
      const text = normalizeImageToTextAnswer(created.solution?.text || created.solution?.answers?.[0]);
      log(`인식(동기): "${text}"`);
      return text;
    }
    if (!created.taskId) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (result.status === 'ready') {
      const text = normalizeImageToTextAnswer(result.solution?.text);
      log(`인식: "${text}"`);
      return text;
    }
    lastErr = `알 수 없는 status: ${result.status}`;
