...
/** 마지막 로그인 자격증명 — VPN/IP보안 끊김 시 자동 재로그인용 (메모리만) */
let savedCreds = {
  naverId: '',
  naverPw: '',
  openaiApiKey: '',
...
let ipSecurityHit = false;

function isIpSecurityMessage(msg) {
  return /IP\s*보안|IP보안|최초\s*로그인한\s*IP|재로그인\s*필요/i.test(String(msg || ''));
}

function rememberCreds(partial = {}) {
  savedCreds = {
    naverId: String(partial.naverId || savedCreds.naverId || '').trim(),
    naverPw: String(partial.naverPw || savedCreds.naverPw || '').trim(),
    openaiApiKey: String(partial.openaiApiKey || savedCreds.openaiApiKey || '').trim(),
    yesCaptchaClientKey: String(partial.yesCaptchaClientKey || savedCreds.yesCaptchaClientKey || '').trim(),
...
function attachMailDialogGuard(p, log) {
  ...
  attachSafeDialogHandler(p, {
    ...
    onMessage: (msg) => {
      if (!isIpSecurityMessage(msg)) return;
      ipSecurityHit = true;
      status = 'idle';
      lastError = '네이버 메일 IP보안 — VPN IP 변경으로 재로그인 필요';
      log?.('⚠ IP보안 팝업 감지 — 자동 재로그인 예정');
      emit();
    },
  });
}
 ...
 * VPN으로 IP가 바뀌면(IP보안) 자격증명으로 자동 재로그인.
 */
export async function ensureDothomeMailSessionReady({
  ...
  rememberCreds({ naverId, naverPw, openaiApiKey, yesCaptchaClientKey, scratchDir });

  const st = getDothomeMailSessionStatus();
  if (st.loggedIn && !ipSecurityHit) return st;

  if (!ipSecurityHit) {
    const revived = await reviveDothomeMailSession({ sendLog, headless });
    if (revived && !ipSecurityHit) return getDothomeMailSessionStatus();
  }

  const id = String(naverId || savedCreds.naverId || '').trim();
  const pw = String(naverPw || savedCreds.naverPw || '').trim();
  const canLogin = allowLogin && id && pw;

  if (!canLogin) {
    throw new Error(
      ipSecurityHit
        ? '네이버 메일 IP보안: VPN IP가 바뀌어 재로그인이 필요합니다. 「네이버 메일 다시 로그인」을 눌러 주세요.'
        : '네이버 메일 로그인이 필요합니다. 닷홈 탭에서 「네이버 메일 로그인」을 먼저 눌러 주세요.\n'
          + '(VPN으로 IP가 바뀌면 메일은 자동 재로그인합니다)',
    );
  }
  ...
  if (ipSecurityHit) log('IP보안/세션 만료 — 메일 자동 재로그인…');

  ipSecurityHit = false;
  return startDothomeNaverMailLogin({
    naverId: id,
    naverPw: pw,
    openaiApiKey: openaiApiKey || savedCreds.openaiApiKey,
    yesCaptchaClientKey: yesCaptchaClientKey || savedCreds.yesCaptchaClientKey,
    headless,
...
/** VPN 단축키 직후 호출 — 메일 IP보안 끊김 대비 강제 재로그인 */
export async function reloginDothomeMailAfterVpn({
  naverId = '',
  naverPw = '',
  openaiApiKey = '',
  yesCaptchaClientKey = '',
  ...
  const pw = String(naverPw || savedCreds.naverPw || '').trim();
  if (!id || !pw) {
    throw new Error('메일 재로그인용 아이디/비밀번호가 없습니다.');
  }
  log(`VPN 후 메일 재로그인 대기 ${Math.round(waitMs / 1000)}초…`);
  await sleep(waitMs);
  ipSecurityHit = false;
  status = 'idle';
  return startDothomeNaverMailLogin({
...
export async function startDothomeNaverMailLogin({
  ...
  loginPromise = (async () => {
    ...
    try {
      ...
      await ensureBrowser({ headless, log });
      attachMailDialogGuard(page, log);
      log(`네이버 메일 로그인 시작: ${accountId}${forceRelogin ? ' (강제 재로그인)' : ''}`);
...
export async function fetchAuthCodeFromMailSession({
  ...
  // IP보안/세션 만료 시 저장된 계정으로 자동 재로그인
  if (ipSecurityHit || !(await isMailLoggedIn(mailPage))) {
    log(ipSecurityHit
      ? 'IP보안으로 메일 세션 끊김 — 자동 재로그인…'
      : '메일 로그인 확인 실패 — 복구/재로그인 시도…');
    let ok = false;
    if (!ipSecurityHit) {
      ok = await reviveDothomeMailSession({ sendLog });
    }
    if (!ok || ipSecurityHit || !(await isMailLoggedIn(page))) {
      if (!(savedCreds.naverId && savedCreds.naverPw)) {
        status = 'idle';
      ...
      ipSecurityHit = false;
      await startDothomeNaverMailLogin({
        naverId: savedCreds.naverId,
        naverPw: savedCreds.naverPw,
        openaiApiKey: savedCreds.openaiApiKey,
        yesCaptchaClientKey: savedCreds.yesCaptchaClientKey,
