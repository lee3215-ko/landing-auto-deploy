...
export const NAVER_MAIL_INBOX = 'https://mail.naver.com/v2/folders/0';
export const NAVER_MAIL_HOME = 'https://mail.naver.com';
 ...
 */
export async function clickMailRowByHostId(page, hostId) {
  const coords = await page.evaluate((hid) => {
    const id = String(hid || '').trim();
    if (!id) return null;

    const rowSelectors = [
      'ul.list_mail li.mail_item',
      'li.mail_item',
      '#mailList li.mail_item',
      '[class*="mail_list"] li',
    ...
    if (!candidates.length) {
      for (const el of document.querySelectorAll(
        'a.link_subject, strong.tit_subject, a.mail_title_link, .mail_title, a[href*="/INBOX/"], a[href*="/read/"], span, a, div',
      )) {
        const t = (el.textContent || '').replace(/\s+/g, ' ').trim();
        if (!t.includes(id)) continue;
...
/** 열린 메일 본문에서 인증코드 추출 */
export async function extractAuthCodeFromOpenMail(page) {
  return page.evaluate(() => {
    const extract = (text) => {
      ...
      if (alnum && !/dothome|naver|kakao|http|mail/i.test(alnum[0])) return alnum[0];
      return '';
    };
    ...
    for (const el of document.querySelectorAll('.mail_view, .view_mail, #mailView, .cont_view, article, .mail_body')) {
      const c = extract(el.textContent || '');
      if (c) return c;
    }
...
/**
 * 네이버 메일에서 닷홈 인증코드 추출
 * - 기본: 닷홈 메일 세션(미리 로그인한 창) 재사용
 * - legacyBrowser: 구버전 호환용 (새 탭+로그인) — 가급적 사용 금지
 */
export async function fetchDothomeAuthCodeFromNaverMail({
  ...
  if (preferSession) {
    try {
      const {
        ...
        fetchAuthCodeFromMailSession,
      } = await import('./dothome-kakao-mail-session.js');
      const st = getDothomeMailSessionStatus();
      if (st.loggedIn) {
        return fetchAuthCodeFromMailSession({ hostId, timeoutMs, sendLog });
      }
      throw new Error(
        '카카오 메일 로그인이 필요합니다. 닷홈 탭에서 「카카오 메일 로그인」을 먼저 눌러 주세요.',
      );
    } catch (e) {
      // 세션 모듈 오류는 그대로 전달 (레거시 폴백으로 재로그인하지 않음)
      throw e;
    }
  }

  // ── 레거시: 호출 측 browser에 새 탭을 열어 로그인 (비권장) ──
  const log = (msg) => {
  ...
  if (!browser) throw new Error('browser가 없습니다.');
  if (!naverId || !naverPw) throw new Error('네이버 메일 로그인용 아이디/비밀번호가 없습니다.');
