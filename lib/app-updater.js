...
function statusPathFor(token) {
  return path.join(os.tmpdir(), `LandingAutoDeploy_upd_status_${token}.json`);
}

function writeStatusFile(token, data) {
  if (!token) return;
  try {
    fs.writeFileSync(
      statusPathFor(token),
      JSON.stringify({ t: Date.now(), ...data }, null, 0),
      'utf8',
    );
  } catch { /* ignore */ }
}
...
function writeUpdatePs1(scriptPath) {
  // 설치 UI(WinForms) + robocopy + 재실행. Electron Job과 무관하게 실행되어야 함.
  const body = `param(
    [string]$Staging,
    [string]$Install,
    [string]$Exe,
...
$StatusFile = if ($Token) { Join-Path $env:TEMP ("LandingAutoDeploy_upd_status_" + $Token + ".json") } else { "" }
$StartedFlag = if ($Token) { Join-Path $env:TEMP ("LandingAutoDeploy_upd_started_" + $Token + ".flag") } else { "" }
...
function Write-Status([string]$Stage, [int]$Pct, [string]$Message, [string]$State = "run") {
    if (-not $StatusFile) { return }
    try {
        $obj = @{ stage = $Stage; pct = $Pct; message = $Message; state = $State; t = (Get-Date -Format o) } | ConvertTo-Json -Compress
        Set-Content -LiteralPath $StatusFile -Value $obj -Encoding UTF8
    } catch {}
}
function Show-Fail([string]$Message) {
    Write-Log ("FAIL: " + $Message)
    Write-Status "fail" 0 $Message "fail"
    try {
        if ($script:form) { $script:form.Hide() }
        Add-Type -AssemblyName System.Windows.Forms -ErrorAction SilentlyContinue
        [System.Windows.Forms.MessageBox]::Show(
            ($Message + [Environment]::NewLine + [Environment]::NewLine + "로그: " + $Log),
...
$mutex = $null
try {
    $mutex = New-Object System.Threading.Mutex($false, "Global\\LandingAutoDeployUpdateMutex")
    if (-not $mutex.WaitOne(0)) {
        Write-Log "another updater already running - exit"
        exit 0
    }
} catch {}

# === 시작 마커 (Electron이 이 파일을 보고 종료함) ===
try {
    if ($StartedFlag) { Set-Content -LiteralPath $StartedFlag -Value (Get-Date -Format o) -Encoding UTF8 }
} catch {}
Write-Log ("update start token=" + $Token)
Write-Log ("Staging=" + $Staging)
Write-Log ("Install=" + $Install)
Write-Log ("Exe=" + $Exe)
Write-Log ("WaitPid=" + $WaitPid)
Write-Status "wait" 5 "앱 종료 대기 중…" "run"
...
try {
    ...
    $form.Text = "Landing Auto Deploy 업데이트"
    ...
    $form.BackColor = [System.Drawing.Color]::FromArgb(15, 20, 28)
    $title = New-Object System.Windows.Forms.Label
    $title.Text = "업데이트 설치 / 재실행"
    ...
    $lbl.Text = "앱 종료 대기 중…"
    $lbl.ForeColor = [System.Drawing.Color]::FromArgb(200, 220, 240)
    $lbl.Font = New-Object System.Drawing.Font("Malgun Gothic", 10)
    $lbl.Location = New-Object System.Drawing.Point(18, 48)
    $lbl.Size = New-Object System.Drawing.Size(410, 40)
    $bar = New-Object System.Windows.Forms.ProgressBar
    $bar.Location = New-Object System.Drawing.Point(18, 100)
    $bar.Size = New-Object System.Drawing.Size(410, 28)
    ...
    Write-Log ("UI create fail: " + $_.Exception.Message)
}
function Set-Ui([string]$Text, [int]$Pct) {
    try {
        if ($script:lbl) { $script:lbl.Text = $Text }
        if ($script:bar) {
            if ($Pct -lt 0) { $script:bar.Style = "Marquee" }
            else {
    ...
    Write-Status "ui" $Pct $Text "run"
}

Set-Ui "앱 종료 대기 중… (재실행 준비)" 10

$deadline = (Get-Date).AddSeconds(150)
while ((Get-Date) -lt $deadline) {
    if (-not (Get-Process -Id $WaitPid -ErrorAction SilentlyContinue)) { break }
    Set-Ui "앱 종료 대기 중… (PID $WaitPid)" 15
...
if (-not (Test-Path (Join-Path $src $exeName))) {
    Show-Fail "압축 해제된 exe를 찾지 못했습니다.\`nsrc=$src"
    exit 2
}

Set-Ui "파일 복사 중… (잠시만 기다려 주세요)" 55
...
if ($rc -ge 8) {
    Show-Fail "파일 복사 실패 (robocopy $rc).\`n프로그램을 모두 종료한 뒤 zip을 수동으로 덮어써 주세요."
    exit 1
}

# 복사 검증
$srcAsar = Join-Path $src "resources\\app.asar"
$dstAsar = Join-Path $Install "resources\\app.asar"
if ((Test-Path $srcAsar) -and (Test-Path $dstAsar)) {
    $ss = (Get-Item -LiteralPath $srcAsar).Length
    $ds = (Get-Item -LiteralPath $dstAsar).Length
    Write-Log ("asar size src=$ss dst=$ds")
    if ($ss -gt 0 -and $ds -ne $ss) {
        Show-Fail "복사 검증 실패 (app.asar 크기 불일치).\`n수동 zip 덮어쓰기가 필요합니다."
        exit 4
    }
}
if (-not (Test-Path -LiteralPath $Exe)) {
    Show-Fail "설치 후 exe가 없습니다:\`n$Exe"
    exit 3
}
...
try {
    ...
    Show-Fail ("재실행 실패: " + $_.Exception.Message + "\`n직접 실행: " + $Exe)
    exit 5
}
Start-Sleep -Seconds 2
$alive = Get-Process | Where-Object {
    try { $_.Path -and $_.Path.Equals($Exe, [System.StringComparison]::OrdinalIgnoreCase) } catch { $false }
} | Select-Object -First 1
if (-not $alive) {
    # 경로 비교 실패해도 이름만으로 재확인
    $alive = Get-Process -Name "Landing Auto Deploy" -ErrorAction SilentlyContinue | Select-Object -First 1
}
if (-not $alive) {
    Show-Fail "새 버전 프로세스가 시작되지 않았습니다.\`n직접 실행해 주세요:\`n$Exe"
    exit 6
}
Write-Log "update success (restarted)"
Write-Status "done" 100 "업데이트 완료 — 새 버전 실행됨" "ok"
Set-Ui "업데이트 완료! 새 버전 실행됨" 100
Start-Sleep -Seconds 2
try { if ($script:form) { $script:form.Close() } } catch {}
Remove-Item -LiteralPath $PSCommandPath -Force -ErrorAction SilentlyContinue
exit 0
`;
  // PowerShell 5.1 한글 경로/문자열용 UTF-8 BOM
...
function spawnBreakaway(command, args, opts = {}) {
  const base = {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  ...
  const flags = CREATE_BREAKAWAY_FROM_JOB | CREATE_NEW_PROCESS_GROUP | DETACHED_PROCESS
    | (opts.showWindow ? 0 : CREATE_NO_WINDOW);
  try {
    const child = spawn(command, args, { ...base, windowsHide: !opts.showWindow, creationFlags: flags });
    child.unref();
    return child;
  } catch (e1) {
    writeUpdateLog(`spawn breakaway fail: ${e1.message || e1}`);
    try {
      const child = spawn(command, args, base);
      child.unref();
      return child;
    } catch (e2) {
      writeUpdateLog(`spawn fallback fail: ${e2.message || e2}`);
      return null;
    }
  }
}
...
/**
 * Electron Job Object에서 탈출해 업데이터 실행.
 * 콘솔/CMD 창은 띄우지 않음 — PowerShell Hidden + WinForms 진행 UI만 표시.
 */
async function launchDetachedUpdater(scriptPath, stagingDir, installDir, exePath, waitPid, token) {
  const fileArgs = [
    scriptPath,
    stagingDir,
    installDir,
  ...
  // 호스트 콘솔은 Hidden — 스크립트 안 WinForms 창만 보임
  const psArgs = [
    '-NoProfile',
  ...
  const psCmdLine = ['powershell.exe', ...psArgs.map(quoteCmd)].join(' ');
  ...
  // ── 1) 작업 스케줄러: Job Object 완전 우회 (Hidden PowerShell) ──
  try {
    const taskName = `LAD_Upd_${waitPid}_${Date.now()}`.slice(0, 60);
    const soon = new Date(Date.now() + 120000);
    const st = `${String(soon.getHours()).padStart(2, '0')}:${String(soon.getMinutes()).padStart(2, '0')}`;
    writeUpdateLog(`launch via schtasks: ${taskName} ST=${st}`);
    await new Promise((resolve) => {
      const child = spawn('schtasks.exe', [
        '/Create', '/TN', taskName, '/TR', psCmdLine,
        '/SC', 'ONCE', '/ST', st, '/F', '/RL', 'LIMITED',
    ...
    await new Promise((resolve) => {
      const child = spawn('schtasks.exe', ['/Run', '/TN', taskName], { windowsHide: true, stdio: 'ignore' });
      child.on('exit', () => resolve());
      child.on('error', () => resolve());
      setTimeout(resolve, 5000);
    });
    setTimeout(() => {
      spawn('schtasks.exe', ['/Delete', '/TN', taskName, '/F'], { windowsHide: true, stdio: 'ignore' });
    }, 20000);
    if (await tryWait(8000)) {
      writeUpdateLog('updater started via schtasks');
      return;
    }
  } catch (e) {
    writeUpdateLog(`schtasks launch fail: ${e.message || e}`);
  }

  // ── 2) Hidden PowerShell 직접 breakaway (CMD/BAT 없음) ──
  try {
    writeUpdateLog(`launch via hidden powershell breakaway: ${scriptPath}`);
    spawnBreakaway('powershell.exe', psArgs, { showWindow: false });
    if (await tryWait(8000)) {
      writeUpdateLog('updater started via hidden powershell');
      return;
    }
  } catch (e) {
    writeUpdateLog(`hidden powershell launch fail: ${e.message || e}`);
  }

  // ── 3) Start-Process Hidden (중첩 런처) ──
  try {
    const launcherPath = path.join(os.tmpdir(), `landing_upd_launch_${waitPid}.ps1`);
    const argList = psArgs.map((a) => `'${String(a).replace(/'/g, "''")}'`).join(', ');
    fs.writeFileSync(
      launcherPath,
      `\uFEFF$ErrorActionPreference='Continue'\r\n`
    ...
    writeUpdateLog(`launch via Start-Process Hidden: ${launcherPath}`);
    spawnBreakaway('powershell.exe', [
      '-NoProfile', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File', launcherPath,
    ], { showWindow: false });
    if (await tryWait(8000)) {
      writeUpdateLog('updater started via Start-Process Hidden');
      return;
    }
  } catch (e) {
  ...
  try {
    ...
    writeUpdateLog(`launch via cmd start /B: ${batPath}`);
    spawnBreakaway('cmd.exe', ['/c', batPath], { showWindow: false });
  } catch (e) {
    writeUpdateLog(`cmd start /B launch fail: ${e.message || e}`);
  }
}

async function waitForUpdaterStarted(token, timeoutMs = 35000) {
  const flagPath = path.join(os.tmpdir(), `LandingAutoDeploy_upd_started_${token}.flag`);
  const marker = `update start token=${token}`;
  let baselineLen = 0;
  try {
  ...
  while (Date.now() - t0 < timeoutMs) {
    try {
      if (fs.existsSync(flagPath)) {
        writeUpdateLog(`started flag ok: ${flagPath}`);
        return true;
      }
    } catch { /* ignore */ }
    try {
      const cur = fs.readFileSync(UPDATE_LOG, 'utf8');
      if (cur.length > baselineLen && cur.slice(baselineLen).includes(marker)) {
        writeUpdateLog('started log marker ok');
        return true;
      }
    } catch { /* ignore */ }
    try {
      const st = JSON.parse(fs.readFileSync(statusPathFor(token), 'utf8'));
      if (st?.state === 'run' || st?.state === 'ok') return true;
    } catch { /* ignore */ }
...
function scheduleApplyUpdate(zipPath) {
  ...
  return extractZip(zipPath, stagingDir).then(() => {
    ...
    return launchDetachedUpdater(scriptPath, stagingDir, installDir, exePath, process.pid, token)
      .then(() => { try { fs.unlinkSync(zipPath); } catch { /* ignore */ } });
  });
}
...
async function setProgressUI(win, done, total, text, meta = null) {
  if (!win || win.isDestroyed()) return;
  try {
    await win.webContents.executeJavaScript(
      `window.setUpdateProgress(${Number(done) || 0}, ${Number(total) || 0}, ${JSON.stringify(text || '')}, ${JSON.stringify(meta || {})});`,
      true,
    );
  } catch { /* ignore */ }
}
...
export async function runStartupUpdateCheck(mainWindow = null) {
  ...
  try {
    ...
    await setProgressUI(progressWin, 0, 100, '다운로드 준비…', {
      stage: 'download',
      detail: `대상 버전 ${info.version}`,
      banner: '다운로드가 시작됩니다. 게이지가 움직여야 정상입니다.',
    });
    ...
    await downloadWithFallbacks(info.downloadUrls || [info.url], zipPath, (done, total) => {
      ...
      const text = total > 0
        ? `다운로드 ${pct}% (${mb} / ${totalMb} MB)`
        : `다운로드 중… (${mb} MB)`;
      setProgressUI(progressWin, done, total || 0, text, {
        stage: 'download',
        detail: 'GitHub에서 파일을 받는 중입니다',
      });
    });

    const sizeMb = Math.round(fs.statSync(zipPath).size / (1024 * 1024));
    sendLog(`다운로드 완료 (${sizeMb} MB)`);
    await setProgressUI(progressWin, 100, 100, '압축 해제 중…', {
      stage: 'extract',
      detail: `다운로드 완료 ${sizeMb} MB — 압축을 푸는 중`,
      banner: '압축 해제 중… 잠시만 기다려 주세요',
    });
    ...
    const token = `${process.pid}_${Date.now()}`;
    writeUpdatePs1(scriptPath);
    writeStatusFile(token, { stage: 'install', pct: 0, message: '설치 스크립트 대기', state: 'pending' });
    writeUpdateLog(`schedule apply install=${installDir} exe=${exePath} pid=${process.pid} token=${token}`);

    await setProgressUI(progressWin, 100, 100, '설치 스크립트 시작 중…', {
      stage: 'install',
      indeterminate: true,
      detail: '별도 설치 창이 곧 뜹니다. 이 창의 게이지가 멈춘 채면 실패입니다.',
      banner: '설치 프로세스 분리 실행 중… (작업 스케줄러 / 독립 프로세스)',
    });

    await launchDetachedUpdater(scriptPath, stagingDir, installDir, exePath, process.pid, token);

    try { fs.unlinkSync(zipPath); } catch { /* ignore */ }
    sendLog(`설치 스크립트 실행 요청 (로그: ${UPDATE_LOG})`);

    const started = await waitForUpdaterStarted(token, 15000);
    if (!started) {
      await setProgressUI(progressWin, 0, 100, '설치 스크립트 시작 실패', {
        stage: 'install',
        failed: true,
        detail: `로그: ${UPDATE_LOG}`,
        banner: '업데이트 실패 — 앱을 종료하지 않습니다. 수동 zip 설치가 필요합니다.',
      });
      throw new Error(
    ...
    await setProgressUI(progressWin, 100, 100, '재실행 준비 완료 — 앱을 종료합니다', {
      stage: 'restart',
      ok: true,
      detail: '별도 「업데이트 설치/재실행」창에서 복사·재실행이 이어집니다.\n이 앱 창은 곧 닫힙니다.',
      banner: '설치 확인됨 → 앱 종료 → 새 버전 자동 실행',
    });
    sendLog('설치 확인됨 — 앱 종료 후 재실행 창에서 이어집니다');
    await new Promise((r) => setTimeout(r, 1800));
    try { if (progressWin && !progressWin.isDestroyed()) progressWin.close(); } catch { /* ignore */ }
    // 파일 잠금 해제를 위해 즉시 종료 (설치 스크립트가 대기 중)
    app.exit(0);
  } catch (e) {
    console.error('[updater]', e);
    writeUpdateLog(`FATAL: ${e.message || e}`);
    try {
      await setProgressUI(progressWin, 0, 100, '업데이트 실패', {
        stage: 'install',
        failed: true,
        detail: formatNetworkError(e),
        banner: '실패 — 앱은 계속 실행됩니다. 로그를 확인하세요.',
      });
    } catch { /* ignore */ }
    try {
      await dialog.showMessageBox({
        type: 'warning',
        buttons: ['로그 폴더 열기', '다운로드 페이지', '닫기'],
        defaultId: 0,
        cancelId: 2,
        title: '업데이트 실패',
        message: '자동 업데이트 실패 (앱은 종료하지 않음)',
        detail: `${formatNetworkError(e)}\n\n로그: ${UPDATE_LOG}`,
        noLink: true,
      }).then(async (r) => {
        if (r.response === 0) await shell.openPath(os.tmpdir());
        if (r.response === 1) {
          await shell.openExternal('https://github.com/lee3215-ko/landing-auto-deploy/releases/latest');
        }
      });
    } catch { /* ignore */ }
    // 실패 시 progressWin 유지(사용자가 상태를 볼 수 있게). 20초 후 닫기
    setTimeout(() => {
      try { if (progressWin && !progressWin.isDestroyed()) progressWin.close(); } catch { /* ignore */ }
    }, 20000);
  }
