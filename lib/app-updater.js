...
function writeUpdatePs1(scriptPath) {
  // Electron 종료 후에도 살아남도록 Start-Process로 띄움.
...
/**
 * Electron Job Object에 묶이지 않도록 Start-Process로 업데이터를 분리 실행.
 * (spawn + app.exit 만 쓰면 자식 PowerShell이 같이 죽는 경우가 많음)
 */
function launchDetachedUpdater(scriptPath, stagingDir, installDir, exePath, waitPid) {
  ...
  const launcherPath = path.join(os.tmpdir(), `landing_upd_launch_${process.pid}.ps1`);
  const launcherBody = [
    "$ErrorActionPreference = 'Continue'",
  ...
  writeUpdateLog(`launch detached updater via ${launcherPath}`);

  // start 로 콘솔/잡에서 완전 분리 → Electron 종료와 무관하게 설치 스크립트 생존
  const child = spawn(
    'cmd.exe',
    [
      '/c',
...
async function waitForUpdaterLogStarted(timeoutMs = 10000) {
...
function scheduleApplyUpdate(zipPath) {
  ...
  return extractZip(zipPath, stagingDir).then(() => {
    ...
    launchDetachedUpdater(scriptPath, stagingDir, installDir, exePath, process.pid);
...
export async function runStartupUpdateCheck(mainWindow = null) {
  ...
  try {
    ...
    const exePath = path.join(installDir, EXE_NAME);
    writeUpdatePs1(scriptPath);
    writeUpdateLog(`schedule apply install=${installDir} exe=${exePath} pid=${process.pid}`);
    ...
    const started = await waitForUpdaterLogStarted(12000);
    if (!started) {
      throw new Error(
