 ...
 * 여러 방법을 순차로 시도하되, 시작 플래그가 보이면 즉시 중단 (중복 설치 방지).
 */
async function launchDetachedUpdater(scriptPath, stagingDir, installDir, exePath, waitPid, token) {
  ...
  // ── 2) cmd start (새 콘솔) + breakaway ──
  try {
    const batPath = path.join(os.tmpdir(), `landing_upd_${waitPid}.cmd`);
    fs.writeFileSync(
      batPath,
      ['@echo off', `start "LandingAutoDeployUpdate" ${psCmdLine}`, 'del "%~f0" >nul 2>&1'].join('\r\n'),
      'utf8',
    );
    writeUpdateLog(`launch via cmd start: ${batPath}`);
    spawnBreakaway('cmd.exe', ['/c', batPath]);
    if (await tryWait(8000)) {
      writeUpdateLog('updater started via cmd start');
      return;
    }
  } catch (e) {
    writeUpdateLog(`cmd start launch fail: ${e.message || e}`);
  }

  // ── 3) PowerShell Start-Process breakaway ──
  try {
    ...
    if (await tryWait(8000)) {
      writeUpdateLog('updater started via Start-Process');
      return;
    }
  } catch (e) {
  ...
  // ── 4) explorer.exe ──
  try {
    const bat2 = path.join(os.tmpdir(), `landing_upd_exp_${waitPid}.cmd`);
    fs.writeFileSync(bat2, `@echo off\r\n${psCmdLine}\r\ndel "%~f0" >nul 2>&1\r\n`, 'utf8');
    writeUpdateLog(`launch via explorer: ${bat2}`);
    spawnBreakaway('explorer.exe', [bat2]);
  } catch (e) {
    writeUpdateLog(`explorer launch fail: ${e.message || e}`);
  }
}
