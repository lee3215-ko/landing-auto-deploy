import { isNaverProtectionText, pauseForNaverProtection } from './naver-protection-guard.js';
/**
...
export function attachSafeDialogHandler(page, opts = {}) {
  ...
  page.on('dialog', async (dialog) => {
    ...
    state.lastMsg = msg;
    markMetaMissing(msg);
    if (isNaverProtectionText(msg)) {
      state.protectionHit = true;
      state.protectionMsg = String(msg || '').replace(/\s+/g, ' ').trim();
      state.protectionAt = Date.now();
      setTimeout(() => {
        pauseForNaverProtection({
          log: state.log,
          detail: state.protectionMsg || '네이버 보호조치 관련 안내가 감지되었습니다.',
          source: 'naver-dialog',
        }).catch(() => {});
      }, 50);
    }
