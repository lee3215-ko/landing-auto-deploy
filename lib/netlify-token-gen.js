...
async function ensureNetlifySession(account, mode, sendLog, openaiApiKey, scratchDir, chromePort, outputRoot, shouldStop = null) {
  const email = resolveNaverEmail(account);
...
export async function generateNetlifyTokens({
  ...
  if (mode === 'signup' && !openaiApiKey) {
    throw new Error('회원가입 모드: 네이버 캡챠 자동 해결을 위해 OpenAI API Key가 필요합니다. 설정 탭에서 입력하세요.');
  }
  ...
  for (let accountIndex = 0; accountIndex < accounts.length; accountIndex += 1) {
    ...
    await ensureNetlifySession(account, mode, sendLog, openaiApiKey, scratchDir, chromePort, outputRoot, shouldStop);
