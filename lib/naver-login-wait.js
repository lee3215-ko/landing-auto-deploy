...
import { pauseForNaverProtection, pageHasNaverProtection as detectProtectionPage } from './naver-protection-guard.js';
...
export async function pageHasNaverProtection(page) {
  return detectProtectionPage(page);
}
    ...
    if (!announced && (protection || onLogin)) {
      if (protection) {
        ...
        await pauseForNaverProtection({
          log,
          detail: '네이버 로그인 중 보호조치·추가인증이 감지되었습니다. 브라우저에서 보호조치를 해제한 뒤 확인해 주세요.',
          source: 'naver-login',
          forceAlert: true,
        });
        continue;
      ...
      await pauseForNaverProtection({
        log,
        detail: '네이버 보호조치가 아직 해제되지 않았습니다. 브라우저에서 보호조치를 해제한 뒤 확인해 주세요.',
        source: 'naver-login',
      });
      continue;
    }
