...
function extractPageResults(html, siteUrl = '') {
  ...
  // nexearch 카드/스니펫 등 최신 마크업 대비: 대상 호스트 URL 직접 추출
  try {
    const host = normalizeSite(siteUrl).split('/')[0];
    return extractGenericHttpUrls(html, host);
  } catch {
    return [];
  }
}
...
function evaluateSearchHtml(html, siteUrl, query) {
  if (htmlHasNoResults(html) && !/suggest_wrap/i.test(html)) {
    return { indexed: false, message: '미인덱싱', sampleUrl: null, resultCount: 0, query };
  }

  const urls = extractPageResults(html, siteUrl);
  ...
  // 결과 링크 파싱 실패해도 페이지에 도메인·타이틀 카드가 보이면 인덱싱으로 본다
  if (domain) {
    const hostPlain = domain.replace(/\./g, '\\.');
    const visibleHost = new RegExp(`(?:^|>|"|')${hostPlain}(?:<|"|'|\\s|/)`, 'i');
    if (visibleHost.test(html) && !htmlHasNoResults(html)) {
      // suggest_wrap의 source(잘못된 공백 쿼리)만 있는 경우는 제외
      const withoutSuggest = html.replace(/<div[^>]*class="[^"]*suggest_wrap[^"]*"[\s\S]*?<\/div>\s*<\/div>/gi, '');
      if (visibleHost.test(withoutSuggest) || extractGenericHttpUrls(withoutSuggest, domain).length) {
        const sample = extractGenericHttpUrls(withoutSuggest, domain)[0] || `https://${domain}/`;
        return {
          indexed: true,
          message: '인덱싱됨 (페이지 표시)',
          sampleUrl: sample,
...
export async function checkNaverIndex(siteUrl) {
  ...
  let status = evaluateSearchHtml(html, siteUrl, query);
  if (status.indexed === true) return status;

  // 네이버 오교정 제안 → 정확한 site: 링크 따라가기 (이미지1 → 이미지2)
  ...
  if (followUrl) {
    try {
      ...
      if (followed.indexed === true) {
        return {
          ...followed,
          message: followed.message === '인덱싱됨'
            ? '인덱싱됨 (제안 링크)'
            : `${followed.message} (제안 링크)`,
          suggestFollowed: true,
        };
      }
      // 제안 링크 이후에도 없으면 그 결과를 우선
      status = { ...followed, suggestFollowed: true };
    } catch (e) {
      return {
        indexed: null,
