  ...
  try {
    ...
    if (!ip) {
      return {
        ...
        host,
        error: 'DNS A레코드 없음',
        tip: '호스팅 미개통(서브도메인 DNS 없음). 이 계정은 버리고 새로 가입하세요. (다시 배포 불가)',
      };
    }
    ...
    return {
      ...
      tip: missing
        ...
        : 'DNS 조회 실패. 네트워크를 확인한 뒤 다시 시도하세요.',
    };
  }
}
 ...
 * 호스트 후보 (접속 우선순위):
 * 1) UI에서 지정한 값
 * 2) {ftpId}.dothome.co.kr — 회선에 따라 공용 FTP(211…)는 막히고 서브도메인(112…)만 되는 경우 많음
 * 3) ftp.dothome.co.kr (공용)
 * ※ expandHostsWithIps가 뒤에 IP를 붙이므로, 서브도메인 IP가 공용 IP보다 먼저 시도됨
 */
export function buildFtpHostCandidates(ftpUser, ftpHost) {
  ...
  push(preferred);
  // 공용 ftp.dothome.co.kr보다 서브도메인 우선 (발행 결과는 동일)
...
function formatFtpHelp(errors) {
  ...
  if (/ENOTFOUND|Non-existent|EAI_AGAIN/i.test(joined)) {
    tips.push('· 서브도메인 DNS 없음 → 호스팅 미개통. 이 계정은 버리고 새로 가입하세요. (다시 배포 불가)');
  }
...
export async function uploadSiteViaFtp({
  ...
  // 서브도메인·그 IP를 공용 ftp / 211… 보다 앞에 유지
  hosts = prioritizeSubdomainFtpHosts(hosts, user);
  relay(sendLog, `FTP 호스트 우선순위: ${hosts.slice(0, 4).join(' → ')}${hosts.length > 4 ? ' …' : ''}`);

  const isPublicFtp = (h) => {
    const s = String(h || '');
    return s === 'ftp.dothome.co.kr' || s === '211.239.120.109';
  };
...
/** 서브도메인 호스트·IP를 공용 ftp.dothome.co.kr / 211.239.120.109 보다 앞에 */
function prioritizeSubdomainFtpHosts(hosts, ftpUser) {
  ...
  const publicHosts = new Set(['ftp.dothome.co.kr', '211.239.120.109']);
  const preferred = [];
  const publicOnes = [];
  const rest = [];
  for (const h of hosts) {
    ...
    else if (/^\d{1,3}(\.\d{1,3}){3}$/.test(s) && s !== '211.239.120.109') {
      // 서브도메인 resolve IP 등 — 공용 IP가 아니면 앞쪽
      preferred.push(s);
    } else rest.push(s);
  }
  const out = [];
...
export async function deployDothomeSite({
  ...
  // 가입 직후 DNS 전파 대기 (서브도메인 ENOTFOUND 방지)
  {
    const ready = await waitForDothomeHostingDns(account.ftpId, sendLog, {
      maxWaitMs: 90_000,
      intervalMs: 5_000,
    });
    if (!ready.ok && ready.status === 'dns_missing') {
      const host = ready.host || `${account.ftpId}.dothome.co.kr`;
      const err = new Error(
        `무료호스팅 미개통(서브도메인 DNS 없음): ${host}\n`
