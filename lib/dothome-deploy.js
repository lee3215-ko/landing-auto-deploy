...
export async function deployDothomeSite({
  ...
  let genResult = null;
  const { resolveExistingZipPath, rewriteSitePublicUrls, saveRewrittenSiteAsSuccessZip } = await import('./source-utils.js');
  ...
  const moveZipAfterFtp = async () => {
    ...
    try {
      // 성공 폴더에는 URL 치환·메타 반영된 「업로드본」을 저장 (원본 Netlify 주소 ZIP이 아님)
      ...
      if (mv?.ok && mv.rewritten) {
        ...
        relay(sendLog, `📦 성공 ZIP 이동(원본): 성공\\${path.basename(mv.path)}${mv.error ? ` · 재압축실패:${mv.error}` : ''}`);
      } else if (mv?.error) {
        relay(sendLog, `⚠ 성공 ZIP 저장 실패(원본 유지): ${mv.error}`);
      }
    } catch (e) {
