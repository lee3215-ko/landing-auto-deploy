...
async function openSiteManagement(page, siteUrl, { preferClick = false, sendLog = null } = {}) {
  ...
  try {
    const { ensureAdvisorPage } = await import('./naver-register.js');
    await ensureAdvisorPage(page, summary, { settleMs: 1500, label: '사이트 요약' });
    return true;
  } catch (e) {
      ...
      for (let ci = 0; ci < candidates.length && !roundSuccess; ci++) {
        ...
        if (/실패|보안절차|자동등록|잘못\s*입력/.test(msg)) {
          ...
          if (nextAlt && stillSame) {
            relay(sendLog, `캡챠 오답 "${answer}" — 대안 시도: ${nextAlt}`);
            await sleep(400);
            continue;
          }
          relay(sendLog, `캡챠/검증 실패 — ${nextAlt ? '이미지 변경됨·' : ''}대안 중단, 새로고침`);
          break;
        }
      }
