...
import { launchBrowser, findSystemEdge } from './puppeteer-launch.js';
import { loginNaverForSearchAdvisor, isNaverLoggedIn, isNaverLoginFormVisible, NaverMassCreatedIdError } from './naver-login-wait.js';
...
/** 서치어드바이저 전용 디버그 포트 (Netlify 9335 / 닷홈메일 9336과 분리) — Edge 사용 */
export const NAVER_DEBUG_PORT = 9334;
/** 서치어드바이저는 Chrome 대신 Edge (사이트 선택·삭제 UI 호환) */
export const NAVER_SESSION_BROWSER = 'edge';
...
/** 진행 중 ensure를 Chrome 종료 시 깨우기 위한 reject */
...
export function getNaverSessionProfileDir() {
  // Edge 전용 프로필 (기존 Chrome 프로필과 분리 — 쿠키 충돌/잠금 방지)
  return defaultUserDataDir
    || path.join(process.cwd(), 'output', 'edge-naver-session');
}
...
/** 현재 세션 메인 탭 (네비게이션/ensure 없이 핸들만) */
export function peekNaverSessionPage() {
  try {
...
/**
 * 디버그 포트(9334) Chrome에 강제 연결.
 * 이미 서치어드바이저에 로그인된 생성용 창을 그대로 씀 (로그인 UI 요구 금지).
 * @returns {Promise<object|null>} puppeteer browser
 */
export async function attachNaverChrome({ onLog = null } = {}) {
  const log = (m) => { if (typeof onLog === 'function') onLog(m); };
  if (await isBrowserAlive()) return browser;

  const reconnected = await connectNaverDebugBrowser(log);
  if (reconnected) {
    browser = reconnected;
    // 기존 탭은 건드리지 않고 핸들만 잡음
    try {
      const pages = await browser.pages();
      const advisor = pages.find((p) => /searchadvisor\.naver\.com/i.test(pageUrl(p)));
      if (advisor && (!page || page.isClosed?.())) page = advisor;
      if (status !== 'ready') {
        status = 'ready';
        if (!accountId) accountId = readSavedSessionAccount(getNaverSessionProfileDir()) || 'session';
        lastError = '';
        emit();
      }
      log(`서치어드바이저 Chrome 연결됨 (탭 ${pages.length}개${advisor ? ', 로그인 탭 유지' : ''})`);
    } catch (e) {
      log(`Chrome 연결됨 (탭 목록 조회 실패: ${e.message || e})`);
    }
    return browser;
  }
  return null;
...
/**
 * 공유 Chrome에 새 탭 생성 (= 브라우저 + 버튼과 동일).
 * 메인(생성 중) 탭은 goto/reload 하지 않음.
 */
export async function openNaverSessionTab({ bringToFront = true, onLog = null } = {}) {
  const b = await attachNaverChrome({ onLog });
  if (!b) {
    throw new Error(
      '서치어드바이저 Edge(포트 9334)에 연결할 수 없습니다. '
      + '「네이버 로그인」으로 연 창이 열려 있는지 확인하세요.',
    );
  }
  const tab = await b.newPage();
  try { await tab.setUserAgent(UA); } catch { /* ignore */ }
  attachPageHandlers(tab);
  if (bringToFront) {
    try { await tab.bringToFront(); } catch { /* ignore */ }
  }
  return tab;
}
...
function attachPageHandlers(p) {
  if (!p) return p;
  // 수동 보드 작업(사이트 선택 후 삭제)이 되도록 삭제 확인은 accept
  // 자동 등록 중에는 naver-register 쪽이 protectDelete=true 로 덮어씀
  attachSafeDialogHandler(p, { protectDelete: false });
...
export async function countAdvisorRegisteredSites(targetPage = null, { forceReload = false } = {}) {
  ...
  try {
    ...
    siteCount = count;
    if (accountId) {
      setKnownNaverSiteCount(accountId, count);
      try { await defaultOnSiteCount?.({ accountId, siteCount: count }); } catch { /* ignore */ }
    }
    emit();
    return count;
  } catch {
...
function cancelPendingSession(reason) {
  const reject = sessionCancel;
  sessionCancel = null;
  if (typeof reject === 'function') {
    try {
      reject(new Error(reason || '네이버 로그인 창이 닫혔습니다. 다시 「네이버 로그인」을 눌러 주세요.'));
    } catch { /* ignore */ }
  }
}
...
function wireBrowserDisconnect(b) {
  ...
  b.on('disconnected', () => {
    ...
    // 보호조치/대량생성 대기 중 창을 닫으면 다음 로그인이 막히지 않게 진행 중 Promise 해제
    cancelPendingSession('네이버 로그인 창이 닫혔습니다. 다시 「네이버 로그인」을 눌러 주세요.');
  });
...
async function connectNaverDebugBrowser(log) {
  ...
  try {
    log(`기존 서치어드바이저 브라우저에 재연결 (포트 ${NAVER_DEBUG_PORT})…`);
    const b = await puppeteer.connect({
      browserURL: `http://127.0.0.1:${NAVER_DEBUG_PORT}`,
      defaultViewport: null,
      // 캡챠 요소 screenshot hang 시 기본 180s 대기를 줄임 (호출측에서도 8s race)
...
/** 프로필을 잠근 고아 Edge/Chrome 프로세스 종료 (Windows) */
function killOrphanChromeForProfile(profileDir, log) {
  ...
  try {
    ...
    const ps = `
$needles = @(${JSON.stringify(needle)}, ${JSON.stringify(needleWin)}, 'edge-naver-session', 'chrome-naver-session')
Get-CimInstance Win32_Process | Where-Object {
  $_.Name -match '^(chrome|msedge)\\.exe$'
} | ForEach-Object {
  $cl = $_.CommandLine
  if (-not $cl) { return }
  $low = $cl.ToLower()
  $portHit = $low.Contains('remote-debugging-port=${NAVER_DEBUG_PORT}')
  foreach ($n in $needles) {
    if ($portHit -or ($n -and $low.Contains([string]$n))) {
...
`;
    ...
    log('프로필을 쓰던 Edge/Chrome 프로세스를 정리했습니다. 다시 실행합니다…');
    return true;
  } catch (e) {
    log(`브라우저 정리 실패: ${e.message || e}`);
    return false;
  }
}
...
async function openNaverBrowser({ headless, profileDir, log }) {
  ...
  try {
    const edgePath = findSystemEdge();
    log(edgePath
      ? `서치어드바이저 Edge 실행… (${edgePath})`
      : '서치어드바이저 Edge 실행… (미설치 시 Chrome 폴백)');
    b = await tryLaunch();
    return wireBrowserDisconnect(b);
  } catch (e) {
...
async function ensureNaverSessionInner(opts = {}) {
  const {
    ...
    forceRelogin = false,
    onLog = null,
    /** true면 보드 이동·사이트 수 집계·빈 탭 정리 생략 (생성 중 새 탭 작업용) */
    preserveTabs = false,
    skipSiteCount = false,
  } = opts;
  ...
  if (!opts.naverAccount?.id || !opts.naverAccount?.pw) {
    throw new Error('네이버 계정이 필요합니다.');
  }

  const wantId = String(opts.naverAccount.id).trim();
  // 설정에서 비번을 바꾼 경우 오버라이드/목록 최신값 우선
  const overridePw = pwOverrides.get(wantId);
  const listPw = listValidNaverAccounts(opts.naverAccounts).find((a) => a.id === wantId)?.pw;
  const wantPw = String(overridePw || listPw || opts.naverAccount.pw || '').trim();
  if (!wantPw) throw new Error('네이버 비밀번호가 필요합니다.');
  const naverAccount = { ...opts.naverAccount, id: wantId, pw: wantPw };
  ...
  // 이미 Chrome이 떠 있으면(생성 중 포함) 계정 재매칭/보드 이동/ensurePage 없이 즉시 반환
  // ※ ensurePage·countAdvisorRegisteredSites 는 메인 탭 reload를 유발하므로 금지
  if (preserveTabs && await isBrowserAlive()) {
    const p = peekNaverSessionPage();
    const id = accountId || savedId || wantId;
    log(`네이버 세션 재사용(탭 유지·무네비게이션): ${id || wantId}`);
    if (!accountId && wantId) accountId = wantId;
    return { browser, page: p, accountId: accountId || wantId };
  }
  ...
  if (await isBrowserAlive()) {
    ...
    if (already && !mustRelogin && savedId === wantId) {
      ...
      if (!preserveTabs) await pruneBlankTabs(p);
      if (!skipSiteCount && !preserveTabs) {
        try {
...
/** 설정 UI에서 비번 변경 시 즉시 반영 (진행 중 로그인에도 적용) */
const pwOverrides = new Map();
/** 계정별 마지막 인식 등록 수 (로그인 스킵용 · config와 동기화) */
const knownSiteCounts = new Map();
let defaultOnSiteCount = null;
let defaultOnMassCreatedId = null;

/** main에서 config 저장 콜백 등록 — 모든 ensureNaverSession 경로에 적용 */
export function setDefaultOnSiteCount(fn) {
  defaultOnSiteCount = typeof fn === 'function' ? fn : null;
}

/** 대량생성 ID 제한 시 확인/목록 삭제 콜백 (Electron dialog 등) */
export function setDefaultOnMassCreatedId(fn) {
  defaultOnMassCreatedId = typeof fn === 'function' ? fn : null;
}

export function setNaverAccountPasswordOverride(id, pw) {
  const key = String(id || '').trim();
  if (!key) return;
  const val = String(pw || '');
  if (val) pwOverrides.set(key, val);
  else pwOverrides.delete(key);
}

export function setKnownNaverSiteCount(id, count) {
  const key = String(id || '').trim();
  if (!key) return;
  if (count == null || Number.isNaN(Number(count))) {
    knownSiteCounts.delete(key);
    return;
  }
  knownSiteCounts.set(key, Math.max(0, Number(count)));
}

export function getKnownNaverSiteCount(id) {
  const key = String(id || '').trim();
  if (!key) return null;
  if (knownSiteCounts.has(key)) return knownSiteCounts.get(key);
  return null;
}

export function isNaverAccountAtLimit(account) {
  const n = account?.siteCount != null
    ? Number(account.siteCount)
    : getKnownNaverSiteCount(account?.id);
  return n != null && Number.isFinite(n) && n >= NAVER_SITE_SOFT_LIMIT;
}

export function listValidNaverAccounts(list) {
  return (Array.isArray(list) ? list : [])
    .map((a) => {
      const id = String(a?.id || '').trim();
      const overridePw = id ? pwOverrides.get(id) : '';
      const pw = String(overridePw || a?.pw || '').trim();
      const fromKnown = getKnownNaverSiteCount(id);
      const siteCountVal = a?.siteCount != null
        ? Number(a.siteCount)
        : (fromKnown != null ? fromKnown : null);
      return {
        id,
        pw,
        siteCount: Number.isFinite(siteCountVal) ? siteCountVal : null,
...
/** 95개 찬 계정은 건너뛰고 사용 가능한 계정 선택 */
export function pickStartNaverAccount(accounts, preferred = null) {
  const list = listValidNaverAccounts(accounts);
  if (!list.length) return null;
  const prefId = String(preferred?.id || preferred || '').trim();
  let startIdx = 0;
  if (prefId) {
    const i = list.findIndex((a) => a.id === prefId);
    if (i >= 0) startIdx = i;
  }
  for (let off = 0; off < list.length; off++) {
    const cand = list[(startIdx + off) % list.length];
    if (!isNaverAccountAtLimit(cand)) {
      // preferred의 최신 비번 오버라이드 반영
      if (prefId && cand.id === prefId && preferred?.pw) {
        const o = pwOverrides.get(cand.id) || preferred.pw;
        return { ...cand, pw: String(o || cand.pw).trim() };
      }
      return cand;
    }
  }
...
export function nextNaverAccount(accounts, currentId) {
  const list = listValidNaverAccounts(accounts);
  if (!list.length) return null;
  ...
  for (let i = 1; i <= list.length; i++) {
    ...
    if (isNaverAccountAtLimit(cand)) continue;
    return cand;
  }
  return null;
}

function isMassCreatedErr(e) {
  return !!(e && (e.code === 'NAVER_MASS_CREATED_ID' || e.name === 'NaverMassCreatedIdError' || e instanceof NaverMassCreatedIdError));
}

export async function ensureNaverSession(opts = {}) {
  if (sessionPromise) {
    // 버튼으로 다시 로그인하거나 강제 재로그인 시, 대기 중 Promise에 묶이지 않게 해제
    if (opts.forceRelogin || opts.forceNewSession) {
      cancelPendingSession('이전 네이버 로그인을 취소하고 다시 시작합니다.');
      try { await sessionPromise.catch(() => {}); } catch { /* ignore */ }
    } else {
  ...
  sessionPromise = (async () => {
    const cancelPromise = new Promise((_, reject) => {
      sessionCancel = reject;
    });

    const run = async () => {
      const log = (msg) => { if (typeof opts.onLog === 'function') opts.onLog(msg); };
      const notifyCount = async (id, n) => {
        if (id && n != null) setKnownNaverSiteCount(id, n);
        const cb = opts.onSiteCount || defaultOnSiteCount;
        try {
          await cb?.({ accountId: id, siteCount: n });
        } catch { /* ignore */ }
      };

      // 설정에 저장된 siteCount 시드
      for (const a of (Array.isArray(opts.naverAccounts) ? opts.naverAccounts : [])) {
        if (a?.id && a.siteCount != null) setKnownNaverSiteCount(a.id, a.siteCount);
      }

      let accounts = listValidNaverAccounts(opts.naverAccounts);
      const fullSkipped = accounts.filter((a) => isNaverAccountAtLimit(a));
      for (const a of fullSkipped) {
        log(`⏭ ${a.id}: 등록 ${a.siteCount}개 ≥ ${NAVER_SITE_SOFT_LIMIT} — 로그인 생략`);
      }

      let account = pickStartNaverAccount(accounts, opts.naverAccount);
      if (!account) {
        if (accounts.length && fullSkipped.length === accounts.length) {
          throw new Error(
            `모든 네이버 계정이 등록 ${NAVER_SITE_SOFT_LIMIT}개 이상입니다.\n`
            + '설정 탭에서 새 계정을 추가하거나, 한도 미만 계정의 개수를 확인하세요.',
          );
        }
        throw new Error('네이버 계정이 필요합니다.');
      }
      if (opts.naverAccount?.id && account.id !== opts.naverAccount.id) {
        log(`시작 계정 ${opts.naverAccount.id}는 한도 도달 → ${account.id}부터 로그인`);
      }

      let result = null;
      let massSwitches = 0;
      while (massSwitches <= accounts.length) {
        try {
          result = await ensureNaverSessionInner({
            ...opts,
            naverAccount: account,
            naverAccounts: accounts,
            forceRelogin: !!opts.forceRelogin || massSwitches > 0,
          });
          break;
        } catch (e) {
          if (!isMassCreatedErr(e)) throw e;
          const blockedId = String(e.accountId || account.id || '').trim();
          log(`🚫 ${blockedId}: 로그인 제한 — 제한사유 대량생성 ID${e.date ? ` (${e.date})` : ''}`);

          const next = nextNaverAccount(
            accounts.filter((a) => a.id !== blockedId),
            blockedId,
          );
          const ask = opts.onMassCreatedId || defaultOnMassCreatedId;
          let decision = { proceed: false };
          if (typeof ask === 'function') {
            try {
              decision = await ask({
                accountId: blockedId,
                reason: e.reason || '대량생성 ID',
                date: e.date || '',
                nextAccount: next,
              }) || { proceed: false };
            } catch {
            ...
            // UI 콜백 없으면 자동으로 다음 계정 시도 + 목록에서 제외
            decision = {
              proceed: true,
              nextAccount: next,
              accounts: accounts.filter((a) => a.id !== blockedId),
            };
            log(`다음 네이버 아이디로 자동 전환: ${next.id}`);
          }
          ...
          const nextAcc = decision.nextAccount || next;
          if (!nextAcc?.id || !nextAcc?.pw) {
            throw new Error(
              `${blockedId}는 대량생성 ID로 제한되었습니다. 설정에 다음으로 쓸 네이버 계정이 없습니다.`,
            );
          }

          if (Array.isArray(decision.accounts)) {
            accounts = listValidNaverAccounts(decision.accounts);
          } else {
            accounts = listValidNaverAccounts(accounts.filter((a) => a.id !== blockedId));
          }
          opts = { ...opts, naverAccounts: accounts };
          account = {
            ...nextAcc,
            id: String(nextAcc.id).trim(),
            pw: String(pwOverrides.get(String(nextAcc.id).trim()) || nextAcc.pw || '').trim(),
          };
          massSwitches += 1;
          log(`대량생성 ID 제외 후 재로그인: ${account.id}`);
        }
      }
      if (!result) throw new Error('네이버 로그인에 실패했습니다.');

      let n = siteCount;
      if (!opts.skipSiteCount && !opts.preserveTabs) {
        try {
          n = await countAdvisorRegisteredSites(result?.page || page, { forceReload: false });
        } catch { /* ignore */ }
      }
      await notifyCount(account.id, n);

      // 95개 이상이면 다음 네이버 아이디로 전환 (한도 100) — 이미 찬 계정은 로그인 없이 스킵
      let switches = 0;
      while (
        n != null
        ...
        setKnownNaverSiteCount(account.id, n);
        const next = nextNaverAccount(accounts, account?.id || result?.accountId || '');
        if (!next) {
          log(`등록 가능한 다음 네이버 계정이 없습니다 (모두 ≥ ${NAVER_SITE_SOFT_LIMIT})`);
          break;
        }
      ...
      // 보호조치 중지 후 재시작이면 서치어드바이저 대시보드로 강제 진입
      try {
        const { consumeAdvisorBoardRestartNeeded } = await import('./naver-protection-guard.js');
        const forceBoard = !!(opts.forceBoard || consumeAdvisorBoardRestartNeeded());
        const p = result?.page;
        if (forceBoard && p && !p.isClosed?.()) {
          log('서치어드바이저 대시보드로 이동 (재시작)');
          await p.goto(BOARD, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
          await new Promise((r) => setTimeout(r, 800));
        }
      } catch { /* ignore */ }
      ...
      return {
        ...
        rotated: switches > 0 || massSwitches > 0,
      };
    };

    try {
      return await Promise.race([run(), cancelPromise]);
    } finally {
      sessionCancel = null;
    }
  })().finally(() => {
...
export async function getNaverSessionBrowser() {
  return attachNaverChrome();
}

/**
 * 진행 중인 ensure를 기다리되, 새 로그인/보드 이동은 시작하지 않음.
 * 수동캡챠가 생성 중 메인 탭을 새로고침하지 않게 하기 위함.
 */
export async function waitForNaverSessionBrowser(timeoutMs = 90000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const b = await getNaverSessionBrowser();
    if (b) return b;
    if (sessionPromise) {
...
export async function closeNaverSession() {
  cancelPendingSession('네이버 세션이 종료되었습니다.');
...
/**
 * 앱 시작 시: 이전에 남아 있는 서치어드바이저용 Chrome(9334)을 닫아
 * 네이버 로그인 창이 기본으로 보이지 않게 함. 「네이버 로그인」 클릭 시에만 다시 연다.
 */
export async function closeOrphanNaverChromeOnStartup() {
  try {
    if (await isBrowserAlive()) {
      await closeNaverSession();
      return;
    }
  } catch { /* ignore */ }

  if (!(await isDebugPortOpen(NAVER_DEBUG_PORT))) {
    browser = null;
    page = null;
    accountId = '';
    status = 'idle';
  ...
  try {
    ...
    try {
      ...
      try {
        killOrphanChromeForProfile(getNaverSessionProfileDir(), () => {});
      } catch { /* ignore */ }
    }
  } catch {
    try {
  ...
  lastError = '';
  siteCount = null;
  clearSavedSessionAccount(getNaverSessionProfileDir());
  emit();
}
