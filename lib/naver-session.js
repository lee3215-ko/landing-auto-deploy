...
/**
 * 공유 Chrome에 새 탭 생성 (메인 세션 탭은 건드리지 않음).
 * 사이트 생성 중 수동캡챠 등 병렬 작업용.
 */
export async function openNaverSessionTab({ bringToFront = true } = {}) {
  if (!(await isBrowserAlive())) {
    const reconnected = await connectNaverDebugBrowser(() => {});
    if (reconnected) browser = reconnected;
  }
  if (!(await isBrowserAlive())) {
    throw new Error('네이버 Chrome 창이 없습니다. 우측 상단 「네이버 로그인」을 먼저 해주세요.');
  }
